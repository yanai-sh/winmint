...
|-- schemas/
|-- apps/
|   |-- setup-shell/          # Native Direct2D FirstLogon splash (AOT)
|   `-- setup-shell-web/      # WebView2 build wizard host
|-- src/
|   `-- runtime/
|       |-- common/
|       |-- image/
...
| `apps/setup-shell/` | Native fullscreen provisioning splash on installed systems | DISM/WIM servicing, wizard profile authoring |
| `apps/setup-shell-web/` | WebView2 build wizard on the build host | DISM/WIM servicing, registry hive edits |
...
| `tools/ui-bridge/` | ISO probe, wizard settings → BuildProfile, dry-run invocation | DISM, offline registry servicing |
...
- `tools/`: developer-only validation, release, bridge, and utility tools.
...
Keep real product logic in the headless PowerShell backend. C# hosts own UI
presentation and read-only runtime state; PowerShell owns DISM, registry hives,
Windows Setup, FirstLogon work, release tooling, validation, and elevation
