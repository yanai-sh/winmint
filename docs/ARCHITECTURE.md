...
**Front ends (not deep modules):** `WinMint.Cli` and Avalonia Wizard are thin clients of **HostCompile** — [DESIGN](DESIGN.md). The Wizard owns navigation and one `WizardSession`; Source, Account, Software, and Review views bind through stage-specific interfaces. Review is projected from one immutable `HostReview` (honesty, diff, quiet copy). `servicing/*.ps1` are ImageServicing adapters, not a second product CLI.
