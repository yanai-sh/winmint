...
4. **Keep-flag expansion** — capabilities/features after metal milestone (**18**); then **19** spike → **20** offline. **Schema `v2` and Profile presets-in-JSON stay out** until an explicit break. **Superseded 2026-08-05 (issue 56):** product-default host preset **`recommended`** is in (expands → Profile ids; Wizard default). Leftover-confidence product cleanup stays out; spike **26** + [ADR-007](ADR-007-cdm-not-primary.md) closed the CDM-as-primary question (not primary).
5. **Acceptance remove-list (grill B4)** — Smoke **acceptance** Profile carries a **small frozen** remove-list (AppX; later also thin capability/feature pins) so prove-out exercises offline remove + digests. Explicit acceptance intent, separate from product **`recommended`**. Schema default for non-compose Cli Profiles remains empty.
...
- **Lasting policy from this ADR + ADR-005/007:** remove-list only; no Profile preset names; product-default **`recommended`** host expansion (issue 56); stay on `winmint.profile/v1` until a breaking change; CDM not primary; Wizard is not a second planner.
