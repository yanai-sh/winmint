...
- Formatter: `.editorconfig` defines UTF-8, final newline, trimmed trailing whitespace, four-space indentation, and LF endings for PowerShell/JSON/XML/YAML/CSS/JS/SVG.
- Linter: PSScriptAnalyzer is configured in `PSScriptAnalyzerSettings.psd1`.
...
- Rust linting: workspace lint `unsafe_op_in_unsafe_fn = "warn"`; `.cargo/config.toml` provides `cargo lint` as `clippy --workspace --all-targets`.
...
- There is no TypeScript path alias or JavaScript module bundler config in the current repository.

## 4) Error and Logging Conventions

- Entry scripts set strict/error behavior explicitly: CLI uses `$ErrorActionPreference = 'Stop'`, `$PSNativeCommandUseErrorActionPreference = $true`, and `Set-StrictMode -Version 2.0`.
...
- Tests are script-based rather than Pester-based; assertions generally collect failures and throw at the end.
- Coverage expectation and coverage tooling are [TODO]; no coverage threshold config was found.
