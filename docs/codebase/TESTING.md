...
- Assertion/mocking tools: custom PowerShell assertion helpers in `tests/contract/ProfileInvariantTests` and Rust `assert_eq!`/`assert!`; no Pester config was found.
- Commands:
...
- Test file placement pattern: repository-level PowerShell contract tests under `tests/contract`; profile fixtures under `tests/profiles`; ignored large fixture roots under `tests/fixtures`.
...
| E2E installer | [TODO] | Full Windows install flow on generated ISO. | VM tooling exists, but no CI E2E workflow was found. |
...
- Common failure mode in tests: integration dry-run requires elevated PowerShell and an ISO fixture; without `-RequireAdmin`, the integration script warns and skips.

## 5) Coverage and Quality Signals

- Coverage tool + threshold: [TODO]; no coverage tool or threshold config found.
- Current reported coverage: [TODO].
- Quality gates in CI: `tests/contract/Test-ProfileInvariants.ps1`, `tools/validation/Validate.ps1`, `cargo check --manifest-path apps/gui/Cargo.toml`, and `cargo test --manifest-path apps/gui/Cargo.toml`.
- Scan output found no production `TODO`, `FIXME`, or `HACK` markers; a direct `rg` search also returned no matches outside excluded artifacts.
