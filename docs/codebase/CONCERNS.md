...
|----------|---------|----------|--------|------------------|
| High | Runtime path migration is broad and currently dirty. | `git status --short` shows deletes under `src/engine`, `src/setup`, `src/agent` and additions under `src/runtime`; `src/runtime/image/WinMint.ps1` is current load entry. | Stale path references can break release bundling, validation, setup staging, or docs. | Run the full validation suite after migration, then search for stale `src/engine`, `src/setup`, and `src/agent` references before merging. |
| Medium | Cloudflare `/cli` wrapper must stay no-legacy/profile-backed. | `cloudflare/winmint/src/index.js` now keeps only profile-backed bootstrap/headless parameters; `AGENTS.md` says configuration flags live only on `new`. | Reintroducing flat flags would recreate a parallel command surface. | Keep `Test-CloudflareWorkerContract.ps1` checking that legacy flags stay absent. |
| Medium | Several orchestration files exceed 500 lines and mix many operational concerns. | Local line-count output: `src/runtime/setup/FirstLogon.ps1` 1620, `src/runtime/image/Private/Image/Unattend.ps1` 959, `src/runtime/image/Reports.ps1` 939, `src/runtime/image/Private/Config/Profile.ps1` 810. | Higher review cost and greater regression risk when changing setup/profile/report behavior. | Extract only when a stable boundary appears; keep tests around each extracted contract. |
...
|-----------|---------------|-------|-----------------|---------------|
| Current branch migration from old layout to `src/runtime/*`. | Architecture consolidation is in progress. | `git status --short`, `docs/Project-Structure.md`, `src/runtime/` | Stale references and release-manifest mistakes. | Complete migration validation before functional changes. |
| Cloudflare CLI wrapper no-legacy policy needs a guard. | Previous headless model exposed flat flags; product stance changed to profile-backed verbs. | `cloudflare/winmint/src/index.js`, `winmint.ps1`, `src/runtime/image/Cli.ps1` | Legacy flags could return accidentally. | Keep a contract assertion that `/cli` does not expose removed flags. |
| Large setup/profile/report modules. | Windows setup orchestration has many tightly ordered phases. | `src/runtime/setup/FirstLogon.ps1`, `src/runtime/image/Reports.ps1`, `src/runtime/image/Private/Config/Profile.ps1` | Local edits can unintentionally affect unrelated phases. | Split only around tested contracts such as profile normalization, cleanup, and report writers. |
...
| Large source files slow review. | Local line-count output listed multiple files over 800 lines. | More context needed per change. | Higher defect probability in setup/profile/report edits. | Extract narrowly around tested seams; avoid broad rewrites. |
| External package installs are sequential in agent modules. | `src/runtime/firstlogon/Start-WinMintAgent.ps1`, `src/runtime/firstlogon/Agent.Runtime.ps1` | Modules run step-by-step and command output is serialized. | FirstLogon duration grows with selected tools. | Only parallelize after proving package managers and installers tolerate it. |
...
|------|-------------|--------------|----------------------|
| `AGENTS.md`, `README.md`, `docs/*.md` | Product contract and user behavior are changing with architecture migration. | Scan recent-history output: `AGENTS.md` 11, `README.md` 9, `docs/Project-Structure.md` 7, `docs/Windows-Debloat-Strategy.md` 7. | Update docs with behavior/schema/test changes in the same commit. |
| CLI/profile engine | Public verb surface and schema v3 profile defaults. | Scan recent-history output: `WinMint-CLI.ps1` 11, profile/assertion tests 8-10, schema 6. | Add or update `Test-CliMatrix.ps1` and profile invariant assertions before changing flags/contracts. |
| Image pipeline/setup staging | DISM, WIM, unattended setup, and path migration are tightly coupled. | Scan recent-history output includes old `src/engine/Private/Image/Unattend.ps1`, `src/engine/Engine.ps1`, `src/engine/Private/Pipeline.ps1`; current files live under `src/runtime/image/`. | Validate path migration, run parser/static guards, and do at least dry-run integration with a fixture ISO. |
...
## 6) Resolved User Decisions

1. Cloudflare `/cli` should have no backward/legacy compatibility flags; keep only the current profile-backed headless/bootstrap surface.
2. `docs/Project-Structure.md` should use the current `assets/runtime/desktop` layout.
3. Release integrity: bootstrap now fails hard when the matching `.sha256` asset is missing.
