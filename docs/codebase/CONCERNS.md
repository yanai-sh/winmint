...
| UI option catalog alignment | Serialized option tokens now live in Rust core and GPUI display rows adapt them. | `crates/winmint-core/src/options.rs`, `apps/gui/src/options.rs`, `schemas/winmint.uiintent.schema.json` | Adding a UI choice in one layer can silently produce unsupported profile values. | Update core options, GPUI options, schema enums, bridge conversion, and UI contract tests together. |
