...
| **Shell timing** | AutoLogon must target the Unattend local user — never `defaultuser0`. OOBE must fully finish before Shell tenure; stall fail-fast remains required ([V1-LESSONS](../design/V1-LESSONS.md)). **Product:** Machine setup (`ILocalAccounts`) deletes leftover `defaultuser0` + profile after Setup (Unattend cannot prevent the OOBE temp account). |
