...
**Quality-cache** lives under `%ProgramData%\WinMint\Servicing\quality-cache\{KB}\{arch}\{sha256}\`. Hit = requested KB in the payload leaf, not directory scan order. Mismatched leaves are quarantined off the hit path (Prepared-media discipline, Catalog bytes). Do not merge the two stores. Dialog, cache resolve, cache write, and `AddQualityUpdates` expand all use that identity. Checkpoint `.msu` is a later Catalog search keyed by the checkpoint KB.
...
`AddQualityUpdates` slipstreams the same-train Microsoft Update Catalog LCU into the **staged** `install.wim` mount when ServicePack Build is behind ([ADR-013](../decisions/ADR-013-catalog-lcu.md)). **Resolve** (`Resolve-WinMintQualityUpdate`) is the interface: WIM identity + Catalog search/details → skip | newest B-release. Live Catalog and fixture HTML are the two adapters; `just check` uses fixtures only. Combined SSU+LCU: extract SSU then `/Add-Package` SSU **then** LCU — classic CAB `.msu` via `expand.exe`, WIM-packaged `.msu` (`MSWIM` magic; large current LCUs) via `dism /Apply-Image` (expand cannot open them). Checkpoint `.msu` files listed by Catalog stay in the same package folder. A Catalog update’s DownloadDialog may list the combined LCU **and** a checkpoint `.msu`; payload identity is the requested KB leaf (`Test-WinMintQualityKbLeaf`), not the first URL or the first file under the quality-cache KB folder. Payload download hosts: `download.windowsupdate.com` or `*.dl.delivery.mp.microsoft.com`. Prepared media is never rewritten.
...
Rejected: v1 `WinMint.ps1` wrap; in-process DISM as default; Profile in kernels; script paths in BuildPlan; per-stage UAC; in-place commit of multi-edition `install.wim`; `IQualityDownload` / BITS-as-port until a second adapter is real (fixture tiny `.msu`). `bitsadmin`, `expand.exe`, and `dism /Apply-Image` (WIM-MSU) stay inside `Resolve-WinMintQualityUpdate.ps1`.
