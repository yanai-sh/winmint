...
bundle into that same session, then launches the packaged WebView2 wizard host. The
...
`irm https://winmint.yanai.sh | iex` and `-Gui` start the packaged WebView2 wizard host
...
builds and packages `WinMintSetupShell.exe` (WebView2 wizard) and
`WinMintSetupShell.Native.exe` (ISO splash); release users do not need local
build toolchains beyond PowerShell 7 and WebView2. Keep developer-only service
source,
