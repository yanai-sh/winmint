...
| Edge | Edge stays installed; `edge-policy-minimal` always applies. WinMint does not automate uninstall and does not present a keep/remove Edge choice (`keep.edge` is always true; `-KeepEdge` is a no-op). | Users may uninstall Edge manually via Settings when DMA unlocks that UI. Never patch `IntegratedServicesRegionPolicySet.json`. **Never** remove WebView2 / Edge *runtime* infrastructure. Do not fail acceptance because Edge remains. |
...
| Windows Search tuning | SearchIndexer can be noisy. | Do not disable local search or the Search service. The Copilot hardware-key chord binds to native Search; keep the platform indexer available for Start/Settings. |
