...
Product `/implement` may proceed from [TICKETS](docs/TICKETS.md) — **one ticket per session**. Apply `ready-for-agent` only when starting that ticket. Next open card: **Wizard packages UI** (deferred) or maintainer pick — [ADR-006](docs/decisions/ADR-006-post-keepflag-sequencing.md).
