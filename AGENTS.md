...
**FirstLogon runtime load order (setup phase):** `FirstLogon.ps1` → `FirstLogon.Context.ps1` (`Set-WinMintFirstLogonContext`) → `FirstLogon.Support.ps1` (loads `WinMint.Runtime.Common.ps1` and split setup modules) → `FirstLogon.Transaction.ps1` → `FirstLogon.Runtime.ps1` (`Invoke-WinMintFirstLogonSetupPhase`).
