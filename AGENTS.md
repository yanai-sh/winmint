...
Product `/implement` may proceed from [TICKETS](docs/TICKETS.md) — **one ticket per session**. Apply `ready-for-agent` only when starting that ticket. Next open card: **20** (Offline ImageServicing capabilities from Profile — [ADR-006](docs/decisions/ADR-006-post-keepflag-sequencing.md)).
