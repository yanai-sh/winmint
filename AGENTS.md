...
Product `/implement` may proceed from [TICKETS](docs/TICKETS.md) — **one ticket per session**. Apply `ready-for-agent` only when starting that ticket. Next open card: **19** (Keep-flag capabilities / features matrix spike — [ADR-006](docs/decisions/ADR-006-post-keepflag-sequencing.md); metal milestone **18** done).
