...
Product `/implement` may proceed from [TICKETS](docs/TICKETS.md) — **one ticket per session**. Apply `ready-for-agent` only when starting that ticket. Next open card: **none** in the 17–20 sequence (deferred backlog: WSL / Wizard packages UI / ExitWindowsEx — [ADR-006](docs/decisions/ADR-006-post-keepflag-sequencing.md)).
