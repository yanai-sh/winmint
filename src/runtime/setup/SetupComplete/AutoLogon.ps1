...
# Catalog runs this early (before toolchain) and again as autologon-stamp-final.

function Invoke-ScAutoLogonStamp {
    param(
        [ValidateSet('early', 'final')]
        [string]$Phase = 'early'
    )
    ...
    $artifactPath = Join-Path $logDir 'SetupComplete_AutoLogon.json'
    $finalArtifactPath = Join-Path $logDir 'SetupComplete_AutoLogon.final.json'

    if ($accountMode -ne 'Local' -or -not $autoLogon) {
        $result.skipped = $true
        $result.reason = 'account.autoLogon not enabled for Local account'
        $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $artifactPath -Encoding UTF8
        if ($Phase -eq 'final') {
            $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $finalArtifactPath -Encoding UTF8
        }
        Write-ScLog "Skipping Winlogon autologon stamp ($Phase): Local+autoLogon not selected."
        return
    }
    if ([string]::IsNullOrWhiteSpace($userName) -or [string]::IsNullOrEmpty($password)) {
        ...
        Write-ScLog "Winlogon autologon stamp ($Phase) fail-closed: missing userName/password in setup profile."
        throw "Local+autoLogon requires account.userName and account.password for Winlogon stamp ($Phase)."
    }
    ...
    try {
        ...
        $afterUser = [string]$wlAfter.DefaultUserName
        $hasPassword = (-not [string]::IsNullOrEmpty([string]$wlAfter.DefaultPassword))
        $result.after = [ordered]@{
            DefaultUserName = $afterUser
            ...
            hasDefaultPassword = $hasPassword
        }

        if ($afterUser -ne $userName -or -not $hasPassword -or [string]$wlAfter.AutoAdminLogon -ne '1') {
            $result.stamped = $false
            $result.failed = "verify-after-stamp failed: DefaultUserName='$afterUser' (want '$userName'), hasDefaultPassword=$hasPassword, AutoAdminLogon=$([string]$wlAfter.AutoAdminLogon)"
            $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $artifactPath -Encoding UTF8
            if ($Phase -eq 'final') {
                $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $finalArtifactPath -Encoding UTF8
            }
            Write-ScLog "Winlogon autologon stamp ($Phase) verify failed: $($result.failed)"
            throw "Winlogon Autologon stamp ($Phase) did not stick: $($result.failed)"
        }

        $result.stamped = $true
        $beforeUser = [string]$result.before.DefaultUserName
        if ($beforeUser -ieq 'defaultuser0' -or ($beforeUser -and $beforeUser -ne $userName)) {
            Write-ScLog "Winlogon autologon restamped ($Phase): DefaultUserName '$beforeUser' -> '$userName' (cleared AutoLogonCount)."
        }
        else {
            Write-ScLog "Winlogon autologon stamped ($Phase) for '$userName' (AutoAdminLogon=1, AutoLogonCount cleared)."
        }
    }
    catch {
        if ($_.Exception.Message -match 'did not stick|Local\+autoLogon requires') { throw }
        $result.failed = [string]$_.Exception.Message
        Write-ScError "Autologon stamp failed ($Phase): $_"
        $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $artifactPath -Encoding UTF8
        if ($Phase -eq 'final') {
            $result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $finalArtifactPath -Encoding UTF8
        }
        throw "Winlogon Autologon stamp ($Phase) failed: $($_.Exception.Message)"
    }
...
function Invoke-ScAutoLogonStampFinal {
    Invoke-ScAutoLogonStamp -Phase 'final'
}
