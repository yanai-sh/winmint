...
function Set-DefaultUserWindowsTerminalDelegation {
    param([Parameter(Mandatory)][string[]]$Roots)

    foreach ($root in $Roots) {
        Invoke-DefaultUserRegistrySet -Entries @(
            @{ Path = "$root\Console\%%Startup"; Name = 'DelegationConsole'; Type = 'REG_SZ'; Data = '{2EACA947-7F5F-4CFA-BA87-8F7FBEEFBE69}' },
            @{ Path = "$root\Console\%%Startup"; Name = 'DelegationTerminal'; Type = 'REG_SZ'; Data = '{E12CFF52-A866-4C77-9A90-F570A7AA2C6B}' }
        )
    }
}
...
$scripts = @(
    ...
    {
        Set-DefaultUserWindowsTerminalDelegation -Roots @('HKU\.DEFAULT', 'HKU\DefaultUser')
    }
)
