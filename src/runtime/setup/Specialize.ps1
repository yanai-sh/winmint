...
function Disable-SpecializeAutoTimeZone {
    Set-SpecializeRegistryValue -Path 'HKLM\SYSTEM\CurrentControlSet\Services\tzautoupdate' -Name Start -Type REG_DWORD -Data 4
    try { Stop-Service -Name tzautoupdate -ErrorAction SilentlyContinue } catch { }
    try { Set-Service -Name tzautoupdate -StartupType Disabled -ErrorAction Stop } catch {
        "Auto Time Zone Updater service disable failed during specialize: $_" | Out-File (Join-Path $logDir 'Specialize_errors.log') -Append
    }
}
...
$restoreLocationServices = Get-SpecializeNestedSetupProfileBool -Section 'regional' -Nested 'dmaInterop' -Name 'restoreLocationServices' -Default $true
$defaultLockScreenPath = 'C:\Windows\Web\Screen\WinMint-Lock.jpg'
...
$scripts = @(
    ...
    {
        Invoke-SpecializeRegistrySet -Entries @(
            @{ Path = 'HKLM\SOFTWARE\Policies\Microsoft\Windows\Personalization'; Name = 'LockScreenImage'; Type = 'REG_SZ'; Data = $defaultLockScreenPath }
        )
    }
...
foreach ($s in $scripts) {
    try { & $s } catch { $errors.Add("Specialize: $_") | Out-Null }
}
if ($errors.Count -gt 0) {
    ($errors.ToArray() -join "`n") | Out-File (Join-Path $logDir 'Specialize_errors.log') -Append
}
