...
function Get-WinMintSetupActionCatalog {
    ...
    return @(
        ...
        [pscustomobject]@{
            ...
            Phase = 'setupComplete'
            RelativePath = 'SetupComplete\SystemHygiene.ps1'
            FunctionName = 'Invoke-ScViveToolOverrides'
            Title = 'Apply targeted ViVeTool feature overrides'
            Kind = 'setup-action'
            Default = $true
            Requires = @('setupComplete.disableVirtualDesktopFlyout')
            SuppressedBy = @('setupComplete.disableVirtualDesktopFlyout=false')
            UserControlled = $true
            Changes = @('Disable the virtual desktop switch flyout when selected shell layers require it')
            Artifacts = @('SetupComplete.log')
            Reversible = $true
        }
        [pscustomobject]@{
