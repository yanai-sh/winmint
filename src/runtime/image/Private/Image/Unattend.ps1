...
function Get-WinMintUnattendOobeShellSetupNode {
    param(
        [Parameter(Mandatory)][System.Xml.XmlDocument]$XmlDoc,
        [Parameter(Mandatory)][System.Xml.XmlNamespaceManager]$NsMgr
    )

    $shellNode = $XmlDoc.SelectSingleNode(
        '//u:settings[@pass="oobeSystem"]/u:component[@name="Microsoft-Windows-Shell-Setup"]',
        $NsMgr)
    if (-not $shellNode) {
        throw 'Autounattend template is missing Microsoft-Windows-Shell-Setup in the oobeSystem pass.'
    }
...
function Install-Autounattend {
    ...
    if ($AccountMode -eq 'Local' -and $AutoLogon -and $TargetPass) {
        $shellNode = Get-WinMintUnattendOobeShellSetupNode -XmlDoc $xmlDoc -NsMgr $nsMgr
        if ($shellNode) {
            $xmlNs = 'urn:schemas-microsoft-com:unattend'
    ...
    Invoke-Action 'Writing autounattend.xml and setup scripts onto the ISO' {
        ...
        if (Test-Path -LiteralPath $bundleDir) {
            ...
            Invoke-WinMintSetupPayloadStaging `
                ...
                -SetupPlan $SetupPlan `
                -ImageArch $ImageArch `
                -LiveInstallAudit $LiveInstallAudit | Out-Null
        }
        else {
