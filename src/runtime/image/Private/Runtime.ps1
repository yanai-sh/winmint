...
function Test-WinMintGitHubApiReachable {
    ...
    try {
        $null = Invoke-RestMethod -Uri 'https://api.github.com/rate_limit' -Headers (Get-WinMintGitHubApiHeaders) -TimeoutSec $TimeoutSec -ErrorAction Stop
        return $true
    }
    catch {
        return $false
