...
function Get-WinMintManifestInstallPlanFacts {
    ...
    if ($null -eq $InstallPlan -or -not $InstallPlan.PSObject.Properties['Facts']) {
        throw 'InstallPlan.Facts is required for manifest generation.'
    }
...
function Initialize-WinMintBuildManifest {
    ...
    $script:WinMintBuildManifest = [ordered]@{
        ...
        audit               = [ordered]@{
            schemaVersion = 1
            generatedAt = [string]$BuildDelta.generatedAt
...
function Save-WinMintBuildManifest {
    ...
    if ($null -ne $script:WinMintBuildManifest.audit.records) {
        $script:WinMintBuildManifest.audit.buildDeltaCount = @($script:WinMintBuildManifest.audit.records).Count
    }
