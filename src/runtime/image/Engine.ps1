...
function New-WinMintBuildConfig {
    ...
    $diagnostics = Get-WinMintProfileSetting $BuildProfile 'diagnostics' @{}
    $wslRuntimeValidation = [string](Get-WinMintProfileSetting $diagnostics 'wslRuntimeValidation' 'full')
    if ($wslRuntimeValidation -notin @('full', 'skip')) {
        throw "diagnostics.wslRuntimeValidation must be 'full' or 'skip'; got '$wslRuntimeValidation'."
    }

    $profileName = [string](Get-WinMintProfileSetting $BuildProfile 'profileName' 'WinMint')
    $keepBlock = Get-WinMintProfileKeepBlock -BuildProfile $BuildProfile
    $keepEdge = $keepBlock.Edge
    $keepGaming = $keepBlock.Gaming
    $keepCopilot = $keepBlock.Copilot
    ...
    $exportHostDrivers = (Test-WinMintDriverSourceUsesHostExport -Source $driverSource) `
        -or [bool](Get-WinMintProfileSetting $drivers 'exportHostDrivers' $false)
    $hostMirrorFilter = 'full'
    if ($exportHostDrivers) {
        $hostMirrorFilter = [string](Get-WinMintProfileSetting $drivers 'hostMirrorFilter' 'setup-critical')
        if ($hostMirrorFilter -notin @('full', 'setup-critical')) { $hostMirrorFilter = 'setup-critical' }
        if (Test-WinMintDriverSourceUsesHostExport -Source $driverSource) { $hostMirrorFilter = 'full' }
    }
    $wsl2Distros = @(ConvertTo-WinMintProfileStringArray (Get-WinMintProfileSetting $wsl 'distros' @()))
    $dotfilesBlock = Get-WinMintProfileSetting $development 'dotfiles' $null
    $dotfilesRepository = ''
    $dotfilesRef = 'main'
    $dotfilesInstallScript = ''
    if ($null -ne $dotfilesBlock) {
        $dotfilesRepository = [string](Get-WinMintProfileSetting $dotfilesBlock 'repository' '')
        $dotfilesRef = [string](Get-WinMintProfileSetting $dotfilesBlock 'ref' 'main')
        if ([string]::IsNullOrWhiteSpace($dotfilesRef)) { $dotfilesRef = 'main' }
        $dotfilesInstallScript = [string](Get-WinMintProfileSetting $dotfilesBlock 'installScript' '')
    }
    ...
    $resolvedTweaks = Resolve-WinMintBuildTweaksFromProfile -BuildProfile $BuildProfile
    $resolvedPrivacy = Resolve-WinMintBuildPrivacyFromProfile -BuildProfile $BuildProfile
    $tweakDarkMode = [bool]$resolvedTweaks.DarkMode
    $tweakFileExtensions = [bool]$resolvedTweaks.FileExtensions
    $tweakStickyKeysOff = [bool]$resolvedTweaks.StickyKeysOff
    $tweakHardwareBypass = [bool]$resolvedTweaks.HardwareBypass
    $tweakDmaInterop = [bool]$resolvedTweaks.DmaInterop
    $updatePolicy = [string]$resolvedTweaks.UpdatePolicy
    $privacyTelemetry = [bool]$resolvedPrivacy.TelemetryHardening
    $privacyAdvertisingDisabled = [bool]$resolvedPrivacy.AdvertisingIdDisabled
    $privacyLocation = [bool]$resolvedPrivacy.Location
    $privacyActivityHistoryDisabled = [bool]$resolvedPrivacy.ActivityHistoryDisabled
    ...
    $tweakContext = New-WinMintTweakContext `
        ...
        -PrivacyAdvertisingIdDisabled $privacyAdvertisingDisabled `
        -PrivacyActivityHistoryDisabled $privacyActivityHistoryDisabled `
    ...
    $removalSettings = Get-WinMintProfileRemovalSettingsFromKeep -Keep $keepBlock
    $baseAppxRemovalPrefixes = @(Get-WinMintEffectiveAppxRemovalPrefix -Settings $removalSettings)
    $appxRemovalPrefixes = @(Resolve-WinMintBuildAppxRemovalPrefixes `
        -BasePrefixes @($baseAppxRemovalPrefixes) `
    ...
    [pscustomobject]@{
        ...
        HostMirrorFilter = $hostMirrorFilter
        ...
        Tweaks = [pscustomobject]@{
            ...
            StickyKeysOff = $tweakStickyKeysOff
            HardwareBypass = $tweakHardwareBypass
            DmaInterop = $tweakDmaInterop
            UpdatePolicy = $updatePolicy
        }
        Privacy = [pscustomobject]@{
            TelemetryHardening = $privacyTelemetry
            AdvertisingIdDisabled = $privacyAdvertisingDisabled
            Location = $privacyLocation
            ActivityHistoryDisabled = $privacyActivityHistoryDisabled
        }
        Drivers = [pscustomobject]@{ Source = $driverSource; Path = $driverPath }
        Diagnostics = [pscustomobject]@{
            RetainFirstLogonArtifacts = [bool](Get-WinMintProfileSetting $diagnostics 'retainFirstLogonArtifacts' $false)
            ProvisioningShellDwellMs = [int](Get-WinMintProfileSetting $diagnostics 'provisioningShellDwellMs' 0)
            WslRuntimeValidation = $wslRuntimeValidation
            VmGuestBasicConsole = [bool](Get-WinMintProfileSetting $diagnostics 'vmGuestBasicConsole' $false)
        }
        ...
        SetupScripts = @(
            ...
            'FirstLogon.Host.ps1', 'FirstLogon.Desktop.ps1',
            'FirstLogon.Region.ps1', 'FirstLogon.Cleanup.ps1', 'WindowsTerminal.Profiles.ps1',
            'FirstLogon.Transaction.ps1', 'FirstLogon.Runtime.ps1', 'WinMint.RuntimeState.ps1', 'WinMintSetupShell.Status.ps1',
            'WinMint.Diagnostics.ps1', 'ProvisioningGuard.ps1',
            'setup-shell', 'WinMintAgent'
        )
...
function Test-WinMintBuildPrerequisite {
    ...
    if (-not $profileOnlyDryRun -and -not [string]::IsNullOrWhiteSpace([string]$Config.Architecture)) {
        try {
            $publishFolder = Resolve-WinMintSetupShellPublishFolder -ImageArch ([string]$Config.Architecture)
            $binRoot = Join-Path (Get-WinMintRepositoryRoot) "assets\runtime\setup\setup-shell\bin\$publishFolder"
            foreach ($rel in @('WinMintSetupShell.Native.exe')) {
                $exePath = Join-Path $binRoot $rel
                if (-not (Test-Path -LiteralPath $exePath -PathType Leaf)) {
                    Add-WinMintBuildPreflightFinding -Severity failure `
                        -Code 'setupShell.exe.missing' `
                        -Message "Native setup shell executable is missing for $($Config.Architecture): $exePath. Run tools\release\Build-WinMintSetupShell.ps1."
                }
            }
        }
        catch {
            Add-WinMintBuildPreflightFinding -Severity failure `
                -Code 'setupShell.exe.archUnsupported' `
    ...
    if (-not $profileOnlyDryRun -and [bool]$PreflightContext.RequireOnlinePayloadCache -and -not (Test-WinMintGitHubApiReachable -TimeoutSec 5)) {
        ...
        if ($cache.Complete) {
            $message = @(
                ...
                'winget, and Cascadia payloads are present,'
    ...
    if ((Test-WinMintDriverSourceUsesHostExport -Source ([string]$Config.Drivers.Source) -or [bool]$Config.ExportHostDrivers) -and $Config.Architecture) {
        $hostArch = Get-BuildHostProcessorArchitecture
        if ($hostArch -ne $Config.Architecture) {
            $message = @(
