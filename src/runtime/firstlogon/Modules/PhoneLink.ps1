...
function Invoke-WinMintAgentPhoneLinkBootstrap {
    [CmdletBinding()]
    param(
    ...
    if ($config.PSObject.Properties['crossDeviceCopyPaste'] -and [bool]$config.crossDeviceCopyPaste) {
        Set-WinMintRegistryDword -Path 'HKCU:\Software\Microsoft\Clipboard' -Name 'EnableClipboardHistory' -Value 1
        Set-WinMintRegistryDword -Path 'HKCU:\Software\Microsoft\Clipboard' -Name 'CloudClipboardAutomaticUpload' -Value 1
