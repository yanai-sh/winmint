...
$setupScriptsRoot = Split-Path -Parent $agentRoot
$diagnosticsHelper = Join-Path $setupScriptsRoot 'WinMint.Diagnostics.ps1'
if (Test-Path -LiteralPath $diagnosticsHelper -PathType Leaf) {
    . $diagnosticsHelper
}
...
$profilePath = Join-Path $agentRoot 'WinMintAgentProfile.json'
...
if (Test-Path -LiteralPath $profilePath -PathType Leaf) {
    $agentProfile = Read-AgentJson -Path $profilePath -Fallback $null
}
$targetArchitecture = if ($null -ne $agentProfile -and $agentProfile.PSObject.Properties['targetArchitecture'] -and -not [string]::IsNullOrWhiteSpace([string]$agentProfile.targetArchitecture)) {
...
if (-not (Test-Path -LiteralPath $profilePath -PathType Leaf)) {
    Write-AgentLog "Agent profile missing: $profilePath"
    $state.steps['profiles'] = @{ status = 'failed'; updatedAt = (Get-Date -Format o); error = "WinMintAgentProfile.json missing: $profilePath" }
    Set-AgentStateValue -State $state -Name 'run' -Value @{
        status = 'failed'
        completedAt = (Get-Date -Format o)
        exitCode = 1
        reason = 'WinMintAgentProfile.json missing'
    }
    Save-AgentState -State $state
    Write-AgentEvent -Type 'run' -Status 'failed' -Step 'profiles' -Message "Agent profile missing: $profilePath"
    Wait-AgentConsoleBeforeClose -Failed $true
    exit 1
}
if ($null -eq $agentProfile) {
    Write-AgentLog "Agent profile unreadable: $profilePath"
    $state.steps['profiles'] = @{ status = 'failed'; updatedAt = (Get-Date -Format o); error = "WinMintAgentProfile.json unreadable: $profilePath" }
    Set-AgentStateValue -State $state -Name 'run' -Value @{
        status = 'failed'
        completedAt = (Get-Date -Format o)
        exitCode = 1
        reason = 'WinMintAgentProfile.json unreadable'
    }
    Save-AgentState -State $state
    Write-AgentEvent -Type 'run' -Status 'failed' -Step 'profiles' -Message "Agent profile unreadable: $profilePath"
    Wait-AgentConsoleBeforeClose -Failed $true
    exit 1
}
