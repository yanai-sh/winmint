...
<#
.SYNOPSIS
    Literal-safe filesystem helpers for WinWS UI PowerShell code.
.NOTES
...
function Get-WinWSPathParentDirectory {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Path
...
function New-WinWSDirectoryLiteral {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$LiteralPath
...
function Set-WinWSUtf8NoBomTextFile {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$LiteralPath,
    ...
    $parent = Get-WinWSPathParentDirectory -Path $LiteralPath
    if (-not [string]::IsNullOrWhiteSpace($parent)) {
        New-WinWSDirectoryLiteral -LiteralPath $parent
    }
