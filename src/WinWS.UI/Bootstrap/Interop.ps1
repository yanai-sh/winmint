...
if (-not ([System.Management.Automation.PSTypeName]'WinWSNative').Type) {
    $src = Get-Content -Raw "$PSScriptRoot\WinWSNative.cs"
    ...
    $cacheDir = Join-Path ([System.IO.Path]::GetTempPath().TrimEnd('\', '/')) 'WinWS_TypeCache'
    $dllPath  = Join-Path $cacheDir "WinWSNative_$srcHash.dll"
    $loaded   = $false
