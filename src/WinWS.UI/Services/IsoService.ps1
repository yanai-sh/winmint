...
function Set-WinWSUiIsoPath {
    ...
    $State.Iso.Path = $Path
    $State.Iso.State = [WinWSIsoState]::Idle
    $State.Iso.Error = ''
    $State.Iso.Architecture = ''
    $State.Iso.Editions = @()
}

function Stop-WinWSUiIsoVerification {
    $iv = Get-WinWSUiIsoVerificationSlot
    ...
    $journalPaths = @()
    if (Get-Command Get-WinWSUiIsoMountJournalFile -ErrorAction SilentlyContinue) {
        $jf = Get-WinWSUiIsoMountJournalFile
    ...
    if (Get-Command Clear-WinWSUiIsoMountJournal -ErrorAction SilentlyContinue) {
        Clear-WinWSUiIsoMountJournal
    }
}

function Sync-WinWSUiIsoControls {
    param([Parameter(Mandatory)][object]$State)

    if (Get-Command Sync-WinWSUiStartControlsFromState -ErrorAction SilentlyContinue) {
        Sync-WinWSUiStartControlsFromState -State $State
    }
    if (Get-Command Update-WinWSUiStateProbe -ErrorAction SilentlyContinue) {
        Update-WinWSUiStateProbe
    }
}

function Start-WinWSUiIsoVerification {
    param(
        [Parameter(Mandatory)][object]$State,
    ...
    if ([string]::IsNullOrWhiteSpace($isoPath) -or -not (Test-Path -LiteralPath $isoPath)) {
        $State.Iso.State = [WinWSIsoState]::Error
        $State.Iso.Error = 'Select a readable Windows ISO.'
        Sync-WinWSUiIsoControls -State $State
        return
    }

    $State.Iso.State = [WinWSIsoState]::Verifying
    $State.Iso.Error = ''
    $State.Iso.Architecture = ''
    $State.Iso.Editions = @()
    Sync-WinWSUiIsoControls -State $State

    if (Get-Command Set-WinWSUiIsoMountJournalPath -ErrorAction SilentlyContinue) {
        Set-WinWSUiIsoMountJournalPath -Path $isoPath
    }
    ...
    $iv.DispatcherPoll.Add_Tick({
        # Do not close over $iv — WPF timer ticks under StrictMode won't see Start-WinWSUiIsoVerification locals.
        $slotOuter = Get-WinWSUiIsoVerificationSlot
        $poll = if ($null -ne $slotOuter) { $slotOuter.DispatcherPoll } else { $null }
        $ok = Invoke-WinWSUiRoutedAction -Source 'IsoVerify.Poll' -Action {
            $ctxInner = Get-WinWSUiAppContextOptional
            $uiState = if ($null -ne $ctxInner) { $ctxInner.State } else { $null }
            $slot = Get-WinWSUiIsoVerificationSlot
            ...
            if ($slot.Job.State -eq 'Running') {
                ...
                Stop-WinWSUiIsoVerification
                try { Dismount-DiskImage -ImagePath $path -ErrorAction SilentlyContinue | Out-Null } catch {}
                $uiState.Iso.State = [WinWSIsoState]::Error
                ...
                Sync-WinWSUiIsoControls -State $uiState
                return
            }
            ...
            if ($verification.Ok) {
                $uiState.Iso.State = [WinWSIsoState]::Verified
                $uiState.Iso.Error = ''
        ...
        if (-not $ok) {
            try { $poll.Stop() } catch {}
            try { Stop-WinWSUiIsoVerification } catch {}
        }
    })
    $iv.DispatcherPoll.Start()
}

function Invoke-WinWSUiBrowseIso {
    param(
        [Parameter(Mandatory)][object]$State,
    ...
    if ($dialog.ShowDialog($Window)) {
        Set-WinWSUiIsoPath -State $State -Path $dialog.FileName
        Start-WinWSUiIsoVerification -State $State -Window $Window
    }
}

function Invoke-WinWSUiClipboardIsoImport {
    param(
        [Parameter(Mandatory)][object]$State,
        [Parameter(Mandatory)][System.Windows.Window]$Window
    )
        ...
        foreach ($drop in $drops) {
            ...
            if ($path.EndsWith('.iso', [System.StringComparison]::OrdinalIgnoreCase) -and
                ...
                if ($path -eq [string]$State.Iso.Path -and
                    ([WinWSIsoState]$State.Iso.State) -in @([WinWSIsoState]::Verified, [WinWSIsoState]::Verifying)) {
                    return
                }
                Set-WinWSUiIsoPath -State $State -Path $path
                Start-WinWSUiIsoVerification -State $State -Window $Window
                return
            }
        }
