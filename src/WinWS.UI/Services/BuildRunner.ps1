...
function Limit-WinWSUiDisplayLine {
    param(
        [string]$Text,
        [int]$Max = 120
    )
...
function Format-WinWSUiBuildHeadline {
    param(
        [string]$Stage,
        [string]$Level,
        [string]$Message
    ...
    $m = Limit-WinWSUiDisplayLine -Text $rawMsg -Max 110
...
function Test-WinWSUiBuildLogPanelWorthy {
    param(
        [string]$Stage,
        [string]$Level
    )
...
function Stop-WinWSUiBuildPump {
...
function Start-WinWSUiBuild {
    ...
    $buildProfile = New-WinWSUiBuildProfile -State $State -IncludeSecrets

    $bd = (Get-WinWSUiAppContext).Build
    ...
    $bd.Job = Start-ThreadJob -ScriptBlock {
        ...
        try {
            ...
            $enginePath = Join-Path $outDir 'src\WinWS\WinWS.ps1'
            & $status 'Loading WinWS engine (this can take a few seconds)…'
            . $enginePath
            Initialize-WinWSEngine -RepositoryRoot $outDir -DryRun:$using:capturedDryRun
            ...
            $result = Start-WinWSBuild -BuildProfile $buildProfile -DryRun:$using:capturedDryRun -ProgressHandler $progressHandler
    ...
    $bd.DispatcherPump.Add_Tick({
        # Do not close over $bd — dispatcher ticks under StrictMode won't see Start-WinWSUiBuild locals.
        $slotPump = Get-WinWSUiBuildSlot
        $pump = if ($null -ne $slotPump) { $slotPump.DispatcherPump } else { $null }
        $ok = Invoke-WinWSUiRoutedAction -Source 'Build.Pump' -Action {
            ...
            while (-not $done -and $slot.Messages.TryDequeue([ref]$msg)) {
                switch ($msg.Kind) {
                    'Log' {
                        Add-WinWSUiBuildLogEntry -State $uiState -Window $win -Level $msg.Level -Text $msg.Text
                    }
                    'Status' {
                        Set-WinWSUiLaunchStatus -Message $msg.Text
                    }
                    'Progress' {
                        $lvl = [string]$msg.Level
                        $stg = [string]$msg.Stage
                        $txt = [string]$msg.Message
                        if (Test-WinWSUiBuildLogPanelWorthy -Stage $stg -Level $lvl) {
                            Add-WinWSUiBuildLogEntry -State $uiState -Window $win -Level $lvl -Text $txt
                        }
                        Set-WinWSUiLaunchStatus -Message (Format-WinWSUiBuildHeadline -Stage $stg -Level $lvl -Message $txt)
                        if (-not [string]::IsNullOrWhiteSpace($stg)) {
                            Update-WinWSUiBuildProgressBar -Window $win -Stage $stg
                        }
                    }
                    'Complete' {
                        ...
                        Set-WinWSUiLaunchStatus -Message $completedStatus
                        ...
                        if ($msg.Success -and $msg.Output) {
                            ...
                            Add-WinWSUiBuildLogEntry -State $uiState -Window $win -Level 'OK' -Text "Output ISO: $leaf  ($dir)"
                        } else {
                            Add-WinWSUiBuildLogEntry -State $uiState -Window $win -Level 'Error' -Text 'Build ended with errors.'
                        }
...
function Add-WinWSUiBuildLogEntry {
    param(
        [object]$State,
        [System.Windows.Window]$Window,
        [string]$Level,
    ...
    Add-WinWSUiLogEntry -State $State -Level $Level -Message $Text

    $panel = $Window.FindName('LogPanel')
    if ($null -eq $panel) { return }

    $textBlock = [System.Windows.Controls.TextBlock]::new()
    $display = Limit-WinWSUiDisplayLine -Text $Text -Max 200
...
function Update-WinWSUiBuildProgressBar {
    param(
        [System.Windows.Window]$Window,
        [string]$Stage
    )
