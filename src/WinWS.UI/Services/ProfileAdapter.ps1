...
$script:WinWSUiFixedEditionNames = @(
    'Windows 11 Home',
    'Windows 11 Pro',
    'Windows 11 Home Single Language'
)

function ConvertTo-WinWSBuildProfileSettings {
    param(
...
function New-WinWSUiBuildProfile {
    param(
        [Parameter(Mandatory)][object]$State,
        [switch]$IncludeSecrets
    )

    if (-not (Get-Command New-WinWSBuildProfile -ErrorAction SilentlyContinue)) {
        . (Join-Path $State.RepositoryRoot 'src\WinWS\WinWS.ps1')
    }
    $settings = ConvertTo-WinWSBuildProfileSettings -State $State -IncludeSecrets:$IncludeSecrets
    return New-WinWSBuildProfile -Settings $settings -IncludeSecrets:$IncludeSecrets
}
