...
# Post-update drift control experiment. This script is not staged, retained, or registered by
# default installs. WinMint does not leave maintenance tasks, services, or drift-control scripts
# on the user's machine; post-install maintenance is user-owned.
$ErrorActionPreference = 'Continue'
$payloadDir = 'C:\Windows\Setup\Scripts'                       # staged config (read-only here)
$logDir     = Join-Path $env:ProgramData 'WinMint\Logs'          # writable log + state
...
if ($privacyAdvertisingId) {
    ...
    # while WinMint AppX policy still provisions Phone Link / Cross Device by default.
