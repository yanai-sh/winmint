...
$winmintWallpaperPath = 'C:\Windows\Web\Wallpaper\WinMint\WinMint-Bloom-OLED.png'
...
$scripts = @(
    ...
    }
    {
        ...
        if (Test-Path -LiteralPath $winmintWallpaperPath) {
            Invoke-DefaultUserRegistrySet -Entries @(
                @{ Path = $desktopKey; Name = 'Wallpaper'; Type = 'REG_SZ'; Data = $winmintWallpaperPath },
