...
<#
.SYNOPSIS
Applies the WinMint Windhawk desktop preset.
...
param(
    ...
    [string]$PresetFile = (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'assets\runtime\desktop\windhawk\preset.json'),
    ...
    [Parameter()]
    [ValidateNotNullOrEmpty()]
    [string]$LogPath = "$env:PUBLIC\Documents\WinMint\WindhawkBootstrap.log",
...
try {
    ...
    $stageDir = Join-Path ([System.IO.Path]::GetTempPath()) ('WinMint-WindhawkPreset-{0}' -f ([guid]::NewGuid().ToString('N')))
