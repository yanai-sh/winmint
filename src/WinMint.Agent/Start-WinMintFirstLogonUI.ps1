...
$hasRaycast = Test-UIAgentModuleEnabled -BuildProfile $agentProfile -Name 'raycast'
...
Add-StepDef -Label 'Raycast' -StateKey 'module:raycast' -Enabled $hasRaycast
...
# ── XAML window ───────────────────────────────────────────────────────────────
$xamlPath = Join-Path $agentRoot 'Start-WinMintFirstLogonUI.xaml'
...
$agentScript = Join-Path $agentRoot 'Start-WinMintAgent.ps1'
    ...
    if ($script:AgentProcess.HasExited) {
        ...
        $window.Dispatcher.Invoke([Action]{
            ...
            if ($script:AgentExitCode -eq 0) {
                ...
                $statusTxt.Text = 'Some selected setup steps did not finish. Review %LOCALAPPDATA%\WinMint\Logs, then retry the failed work from here.'
...
$window.FindName('BtnRestart').Add_Click({
    Start-Process -FilePath 'shutdown.exe' -ArgumentList '/r', '/t', '10', '/c', 'WinMint Setup is restarting to complete configuration.' -WindowStyle Hidden
    $window.Close()
})
