...
$eventLogPath = Join-Path $logDir 'WinWSAgent-events.jsonl'
...
Write-AgentLog 'WinWSAgent start'
Write-AgentEvent -Type 'run' -Status 'starting' -Message 'WinWSAgent start'
...
Invoke-AgentProfileModule -StepName 'profiles' -FunctionName 'Invoke-WinWSAgentProfileBootstrap' -Enabled $true
Invoke-AgentProfileModule -StepName 'liveInstallAudit' -FunctionName 'Invoke-WinWSAgentLiveInstallAuditBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'liveInstallAudit')
Invoke-AgentProfileModule -StepName 'package-managers' -FunctionName 'Invoke-WinWSAgentPackageManagerBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'packageManagers')
Invoke-AgentProfileModule -StepName 'wsl' -FunctionName 'Invoke-WinWSAgentWslBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'wsl')
Invoke-AgentProfileModule -StepName 'git' -FunctionName 'Invoke-WinWSAgentGitBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'git')
Invoke-AgentProfileModule -StepName 'dotfiles' -FunctionName 'Invoke-WinWSAgentDotfileBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'dotfiles')
Invoke-AgentProfileModule -StepName 'flow-everything' -FunctionName 'Invoke-WinWSAgentFlowEverythingBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'flowEverything')
Invoke-AgentProfileModule -StepName 'phone-link' -FunctionName 'Invoke-WinWSAgentPhoneLinkBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'phoneLink')
Invoke-AgentProfileModule -StepName 'tiling-desktop' -FunctionName 'Invoke-WinWSAgentTilingDesktopBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'shell')
Invoke-AgentProfileModule -StepName 'windhawk' -FunctionName 'Invoke-WinWSAgentWindhawkBootstrap' -Enabled (Test-AgentModuleEnabled -Name 'windhawk')

Invoke-AgentProfileModule -StepName 'editors' -FunctionName 'Invoke-WinWSAgentEditorBootstrap' -Enabled (@($agentProfile.editors).Count -gt 0)
...
if ($failed.Count -gt 0) {
    ...
    Write-AgentLog "WinWSAgent failed: $($failed.Count) failed step(s)."
