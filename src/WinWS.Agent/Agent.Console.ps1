...
function Write-AgentLog {
    ...
    "$(Get-Date -Format o) $Message" | Out-File (Join-Path $logDir 'WinWSAgent.log') -Append -Encoding utf8
}
...
function Show-AgentConsoleHeader {
    ...
    if ($script:AgentConsoleReady) {
        $body = @(
            ...
            '[grey]This window can be left alone. Detailed logs are written under[/] [silver]%LOCALAPPDATA%\WinWS\Logs[/][grey].[/]'
        ) -join "`n"
        $null = Format-SpectrePanel -Data $body -Header '[bold dodgerblue1]WinWS FirstLogon[/]' -Border Rounded -Color DodgerBlue1 -Expand |
            Out-SpectreHost | Out-Host
        return
    }
    Write-Host ''
    Write-Host 'WinWS FirstLogon' -ForegroundColor Cyan
    Write-Host 'Applying the selected first-logon preset. Logs are under %LOCALAPPDATA%\\WinWS\\Logs.' -ForegroundColor Gray
    Write-Host ''
}
