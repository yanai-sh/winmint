...
function Set-WinMintAgentEverythingAlphaConfiguration {
    [CmdletBinding()]
    ...
    foreach ($ini in $iniPaths) {
        try {
            Set-WinMintAgentIniValue -Path $ini -Name 'app_data' -Value '1'
            Set-WinMintAgentIniValue -Path $ini -Name 'alpha_instance' -Value '0'
            Set-WinMintAgentIniValue -Path $ini -Name 'service_name' -Value 'Everything'
            Set-WinMintAgentIniValue -Path $ini -Name 'service_pipe_name' -Value '\\.\PIPE\Everything Service'
            Set-WinMintAgentIniValue -Path $ini -Name 'allow_multiple_windows' -Value '0'
            Set-WinMintAgentIniValue -Path $ini -Name 'run_in_background' -Value '1'
            Set-WinMintAgentIniValue -Path $ini -Name 'show_tray_icon' -Value '0'
        }
        catch {
...
function Set-WinMintAgentFlowLauncherEverythingIntegration {
    [CmdletBinding()]
    param([Parameter(Mandatory)][hashtable]$State)
...
function Invoke-WinMintAgentFlowEverythingBootstrap {
    [CmdletBinding()]
    param(
    ...
    Set-WinMintAgentEverythingAlphaConfiguration -State $State
    Install-AgentManifestTool -ToolId 'flow-launcher' -State $State
    Set-WinMintAgentFlowLauncherEverythingIntegration -State $State
