...
function Save-WinWSBuildReport {
    param([Parameter(Mandatory)]$Report)

    $out = Get-WinWSOutputDirectory
    $jsonPath = Join-Path $out 'WinWS-BuildReport.json'
    $mdPath = Join-Path $out 'WinWS-BuildReport.md'
...
function Save-WinWSDryRunArtifacts {
    param(
        [Parameter(Mandatory)]$Config,
    ...
    $setupProfilePath = Join-Path $artifactDir 'WinWSSetupProfile.json'
    $agentProfilePath = Join-Path $artifactDir 'WinWSAgentProfile.json'
    $setupPlanPath = Join-Path $artifactDir 'WinWSSetupPlan.json'
    ...
    $readme = @(
        ...
        '| WinWSSetupPlan.json | Backend setup-phase plan for CLI/UI inspection and audit. |'
        '| WinWSSetupProfile.json | SetupComplete, DefaultUser, FirstLogon, and maintenance profile. |'
        '| WinWSAgentProfile.json | FirstLogon agent profile generated from selected app/desktop/WSL choices. |'
...
function Get-WinWSRegistryTweakGroupValue {
    param(
...
function ConvertTo-WinWSRegistryAuditPath {
...
function ConvertTo-WinWSRegDwordText {
...
function ConvertTo-WinWSRegStringText {
...
function ConvertTo-WinWSRegValueLine {
    param(
        [Parameter(Mandatory)][string]$Name,
    ...
    $regName = if ([string]::IsNullOrWhiteSpace($Name)) { '@' } else { ConvertTo-WinWSRegStringText -Value $Name }
    $regValue = switch ($Type) {
        'REG_DWORD' { ConvertTo-WinWSRegDwordText -Value ([string]$Value) }
        default { ConvertTo-WinWSRegStringText -Value ([string]$Value) }
    }
    return "$regName=$regValue"
}

function New-WinWSRegistryTweakDetail {
    param(
        [Parameter(Mandatory)]$Group,
    ...
    $setOps = @(Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'set' -Default @())
    $removeOps = @(Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'remove' -Default @())
    $rollbackCount = @(
        foreach ($entry in $setOps) {
            if ($null -ne (Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'undo')) { $entry }
        }
        foreach ($entry in $removeOps) {
            if ($null -ne (Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'restore')) { $entry }
        }
    ).Count

    [ordered]@{
        id = [string](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'id')
        description = [string](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'description')
        scope = [string](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'scope' -Default 'offline registry')
        risk = [string](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'risk' -Default 'low')
        reversible = [bool](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'reversible' -Default $false)
        phase = [string](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'phase' -Default 'offline-image')
        intent = [string](Get-WinWSRegistryTweakGroupValue -Group $Group -Name 'intent' -Default '')
        status = $Status
...
function Initialize-WinWSBuildManifest {
    ...
    $script:WinWSBuildManifest = [ordered]@{
        schemaVersion       = 1
        ...
        tweaks              = [ordered]@{
            ...
            registryGroups        = @(
                foreach ($group in @($script:RegistryTweaks)) {
                    ...
                    New-WinWSRegistryTweakDetail -Group $group -Status $status
                }
            )
        }
        drivers             = [ordered]@{
...
function Add-WinWSManifestRegistryTweakEvent {
    param(
        [Parameter(Mandatory)]$Group,
    ...
    if ($null -eq $script:WinWSBuildManifest) { return }
    if (-not $script:WinWSBuildManifest.tweaks.Contains('registryGroups')) {
        $script:WinWSBuildManifest.tweaks['registryGroups'] = @()
    }

    $detail = New-WinWSRegistryTweakDetail -Group $Group -Status $Status -ErrorMessage $ErrorMessage
    $groups = [System.Collections.Generic.List[object]]::new()
    $replaced = $false
    foreach ($existing in @($script:WinWSBuildManifest.tweaks.registryGroups)) {
    ...
    $script:WinWSBuildManifest.tweaks.registryGroups = $groups.ToArray()
}

function New-WinWSTweakRollbackRegContent {
    param([object[]]$Groups)

    $lines = [System.Collections.Generic.List[string]]::new()
    $lines.Add('Windows Registry Editor Version 5.00') | Out-Null
    $lines.Add('') | Out-Null
    $lines.Add('; WinWS best-effort tweak rollback. Review before importing.') | Out-Null
    foreach ($group in @($Groups)) {
        $setOps = @(Get-WinWSRegistryTweakGroupValue -Group $group -Name 'set' -Default @())
        $removeOps = @(Get-WinWSRegistryTweakGroupValue -Group $group -Name 'remove' -Default @())
        $wroteHeader = $false
        foreach ($entry in $setOps) {
            $undo = Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'undo'
            if ($null -eq $undo) { continue }
            if (-not $wroteHeader) {
                $lines.Add('') | Out-Null
                $lines.Add("; $([string](Get-WinWSRegistryTweakGroupValue -Group $group -Name 'id'))") | Out-Null
                $wroteHeader = $true
            }
            $path = ConvertTo-WinWSRegistryAuditPath -Path ([string](Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'path'))
            $name = [string](Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'name')
            $action = [string](Get-WinWSRegistryTweakGroupValue -Group $undo -Name 'action' -Default 'set')
            ...
            if ($action -eq 'delete') {
                ...
                $type = [string](Get-WinWSRegistryTweakGroupValue -Group $undo -Name 'type' -Default ([string](Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'type')))
                $value = [string](Get-WinWSRegistryTweakGroupValue -Group $undo -Name 'value' -Default '')
                $lines.Add((ConvertTo-WinWSRegValueLine -Name $name -Type $type -Value $value)) | Out-Null
            }
            $lines.Add('') | Out-Null
        }
        foreach ($entry in $removeOps) {
            $restore = Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'restore'
            ...
            $path = ConvertTo-WinWSRegistryAuditPath -Path ([string](Get-WinWSRegistryTweakGroupValue -Group $entry -Name 'path'))
            $lines.Add("[$path]") | Out-Null
            foreach ($value in @(Get-WinWSRegistryTweakGroupValue -Group $restore -Name 'values' -Default @())) {
                $lines.Add((ConvertTo-WinWSRegValueLine `
                    -Name ([string](Get-WinWSRegistryTweakGroupValue -Group $value -Name 'name')) `
                    -Type ([string](Get-WinWSRegistryTweakGroupValue -Group $value -Name 'type')) `
                    -Value ([string](Get-WinWSRegistryTweakGroupValue -Group $value -Name 'value')))) | Out-Null
            }
            $lines.Add('') | Out-Null
        }
    }
...
function Save-WinWSTweakAuditArtifacts {
    param([Parameter(Mandatory)][string]$OutputDir)

    if ($null -eq $script:WinWSBuildManifest) { return $null }
    $groups = @($script:WinWSBuildManifest.tweaks.registryGroups)
    $audit = [ordered]@{
        generatedAt = [DateTimeOffset]::Now.ToString('o')
        selected = @($script:WinWSBuildManifest.tweaks.registryGroupsApplied)
        groups = $groups
        summary = [ordered]@{
    ...
    $jsonPath = Join-Path $OutputDir 'WinWS-TweakAudit.json'
    $mdPath = Join-Path $OutputDir 'WinWS-TweakAudit.md'
    $regPath = Join-Path $OutputDir 'WinWS-TweakRollback.reg'
    $audit | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $jsonPath -Encoding UTF8
    Set-Content -LiteralPath $regPath -Value (New-WinWSTweakRollbackRegContent -Groups $script:RegistryTweaks) -Encoding Unicode
...
function Add-WinWSManifestPayload {
    param(
        [Parameter(Mandatory)][string]$Name,
    ...
    foreach ($payload in @($script:WinWSBuildManifest.payloads)) {
    ...
    $script:WinWSBuildManifest.payloads.Add([ordered]@{
        name      = $Name
        sourceUrl = $SourceUrl
        version   = $Version
        sha256    = $Sha256
...
function Get-WinWSDeduplicatedManifestPayload {
...
function Save-WinWSBuildManifest {
    param(
    ...
    $started = [DateTimeOffset]::Parse($script:WinWSBuildManifest.builtAt)
    $script:WinWSBuildManifest.buildDurationSeconds = [math]::Round(([DateTimeOffset]::Now - $started).TotalSeconds, 1)
    $script:WinWSBuildManifest.buildResult = if ($Failed) { 'failed' } elseif ($DryRun) { 'dry-run' } else { 'success' }
    ...
    if (-not [string]::IsNullOrWhiteSpace($OutputIsoPath) -and (Test-Path -LiteralPath $OutputIsoPath)) {
        ...
        $script:WinWSBuildManifest['output'] = [ordered]@{
    ...
    $script:WinWSBuildManifest.payloads = @(Get-WinWSDeduplicatedManifestPayload -Payloads @($script:WinWSBuildManifest.payloads))
    $null = Save-WinWSTweakAuditArtifacts -OutputDir $OutputDir

    $path = Join-Path $OutputDir 'WinWS-BuildManifest.json'
    $script:WinWSBuildManifest | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $path -Encoding UTF8
    return $path
}

function Write-WinWSDryRunArtifactSummary {
