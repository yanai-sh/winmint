...
function Write-WinWSConsoleLine {
...
function Send-WinWSConsoleLogToProgressHandler {
    param(
        [ValidateSet('Info','OK','Warn','Error','Section')]
        [string]$Level = 'Info',
        [Parameter(Mandatory)][string]$Message
    )
    $handler = Get-Variable -Name WinWSProgressHandler -Scope Script -ValueOnly -ErrorAction SilentlyContinue
    if ($null -eq $handler) { return }
    Write-WinWSProgress -Level $Level -Message $Message -ProgressHandler $handler
}

function Log {
    param([Parameter(Mandatory, Position = 0)][string]$Message)
    $ts = Get-Win11IsoLogTimestampPrefix
    Write-WinWSConsoleLine "${ts}[cyan1]>[/] [white]$Message[/]" "> $Message"
    Send-WinWSConsoleLogToProgressHandler -Level Info -Message $Message
...
function LogOK {
    param([Parameter(Mandatory, Position = 0)][string]$Message)
    $ts = Get-Win11IsoLogTimestampPrefix
    Write-WinWSConsoleLine "${ts}[green]+[/] [silver]$Message[/]" "+ $Message"
    Send-WinWSConsoleLogToProgressHandler -Level OK -Message $Message
    Invoke-Win11IsoLogStreamFlushUnlessVerbose
}
...
function LogWarn {
    param([Parameter(Mandatory, Position = 0)][string]$Message)
    $ts = Get-Win11IsoLogTimestampPrefix
    Write-WinWSConsoleLine "${ts}[yellow]![/] [white]$Message[/]" "! $Message"
    Send-WinWSConsoleLogToProgressHandler -Level Warn -Message $Message
    Invoke-Win11IsoLogStreamFlushUnlessVerbose
}
...
function LogErr {
    ...
    if ($Message -is [System.Management.Automation.ErrorRecord]) {
        ...
        Write-WinWSConsoleLine "${ts}[red]x[/] [white]$([Spectre.Console.Markup]::Escape($line))[/]" "x $line"
        Send-WinWSConsoleLogToProgressHandler -Level Error -Message $line
        if (Test-Win11IsoVerboseLogging) {
            if ($Message.InvocationInfo.PositionMessage) {
                Write-WinWSConsoleLine "[dim]$([Spectre.Console.Markup]::Escape($Message.InvocationInfo.PositionMessage))[/]" $Message.InvocationInfo.PositionMessage
            }
            if ($Message.ScriptStackTrace) {
                Write-WinWSConsoleLine "[dim]$([Spectre.Console.Markup]::Escape($Message.ScriptStackTrace))[/]" $Message.ScriptStackTrace
            }
        }
        ...
        $plain = "$Message"
        Write-WinWSConsoleLine "${ts}[red]x[/] [white]$([Spectre.Console.Markup]::Escape($plain))[/]" "x $plain"
        Send-WinWSConsoleLogToProgressHandler -Level Error -Message $plain
        if (Test-Win11IsoVerboseLogging) {
...
function LogDry {
    param([Parameter(Mandatory, Position = 0)][string]$Message)
    $ts = Get-Win11IsoLogTimestampPrefix
    Write-WinWSConsoleLine "${ts}[darkorange3]dry[/] [silver]$Message[/]" "dry $Message"
    Send-WinWSConsoleLogToProgressHandler -Level Info -Message "Dry run: $Message"
    Invoke-Win11IsoLogStreamFlushUnlessVerbose
}
