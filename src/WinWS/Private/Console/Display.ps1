...
function Show-BuildWelcomeHero {
    ...
    $heroPngPath = Join-Path (Get-WinWSRepositoryRoot) 'assets\brand\WinMint.svg'
...
$script:WinWSActionTimingVisibleThresholdSeconds = 60

function Format-WinWSDuration {
...
function Start-PipelinePhase {
    ...
    if ($null -ne $handler) {
        Write-WinWSProgress -Stage $Name -Level Section -Message "Starting $Name" -ProgressHandler $handler
    }
...
function Complete-PipelinePhase {
    ...
    if ($null -ne $script:PipelinePhaseStartedAt -and $script:PipelinePhaseStartedAt.ContainsKey($Name)) {
        ...
        $timer.Stop()
        $message = "$message in $(Format-WinWSDuration -Duration $timer.Elapsed)"
        $script:PipelinePhaseStartedAt.Remove($Name)
    }
    $handler = Get-Variable -Name WinWSProgressHandler -Scope Script -ValueOnly -ErrorAction SilentlyContinue
    if ($null -ne $handler) {
        Write-WinWSProgress -Stage $Name -Level OK -Message $message -ProgressHandler $handler
    }
    $tasks = Get-Variable -Name PipelineTasks -Scope Script -ValueOnly -ErrorAction SilentlyContinue
    if ($null -eq $tasks) { return }
    $task = $tasks[$Name]
...
function Invoke-Action {
    ...
    if ($script:DryRun) { LogDry $Description }
        ...
        try {
            ...
            $timer.Stop()
            if ($completed) {
                $elapsed = Format-WinWSDuration -Duration $timer.Elapsed
                $threshold = [double]$script:WinWSActionTimingVisibleThresholdSeconds
