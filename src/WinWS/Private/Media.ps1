...
function ConvertTo-WinWSComparablePath {
...
function Test-WinWSComparablePathIsUnderRoot {
    <# <summary>True when Candidate is the root folder or a descendant of Root (case-insensitive).</summary> #>
    param(
        [Parameter(Mandatory)][string]$Candidate,
        [Parameter(Mandatory)][string]$Root
    )
    $c = ConvertTo-WinWSComparablePath -Path $Candidate
    $r = ConvertTo-WinWSComparablePath -Path $Root
...
function Test-WinWSMountedImagePath {
    param(
    ...
    $target = ConvertTo-WinWSComparablePath -Path $Path
    foreach ($image in $images) {
        ...
        if ((ConvertTo-WinWSComparablePath -Path $candidate) -eq $target) { return $true }
    }
    return $false
}
...
function Invoke-Cleanup {
    ...
    if ($MountDir -and (Test-Path -LiteralPath $MountDir) -and (Test-WinWSMountedImagePath -Path $MountDir)) {
        try {
...
function Invoke-Win11IsoStartupCleanup {
    ...
    # Any mounted WIM whose mount path lives under %TEMP% (abnormal layouts, not only WinWS_ISO_*).
    try {
        foreach ($image in @(Get-WindowsImage -Mounted -ErrorAction SilentlyContinue)) {
            ...
            if (-not (Test-WinWSComparablePathIsUnderRoot -Candidate $mountPath -Root $temp)) { continue }
    ...
    try {
        foreach ($di in @(Get-DiskImage -ErrorAction SilentlyContinue)) {
            ...
            if (-not (Test-WinWSComparablePathIsUnderRoot -Candidate $img -Root $temp)) { continue }
    ...
    foreach ($dir in @(Get-ChildItem -LiteralPath $temp -Filter 'WinWS_ISO_*' -Directory -ErrorAction SilentlyContinue)) {
        $mountPath = Join-Path $dir.FullName 'mount'
        if ((Test-Path -LiteralPath $mountPath) -and (Test-WinWSMountedImagePath -Path $mountPath)) {
            try { $null = Dismount-WindowsImage -Path $mountPath -Discard -ErrorAction SilentlyContinue } catch {}
        }
    ...
    try { Invoke-WinWSAllBuildCachesMaintenance } catch { Write-Verbose "Invoke-Win11IsoStartupCleanup build caches: $($_.Exception.Message)" }
}
