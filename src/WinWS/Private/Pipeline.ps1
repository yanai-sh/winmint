...
function Convert-WinWSInstallEsdToWim {
    param(
...
function Mount-WinWSIsoToWorkTree {
    param(
    ...
    try {
        ...
        $iso = Mount-DiskImage -ImagePath $SourceIso -Access ReadOnly -PassThru -ErrorAction Stop
        ...
        if (-not $DryRun) {
            Clear-WinWSReadOnlyAttribute -Path $IsoContents
        }
    }
    finally {
...
function Get-WinWSSelectedInstallImage {
    param(
...
function Get-WinWSInstallImagesForBuild {
    param(
        [Parameter(Mandatory)][string]$InstallWim,
    ...
    if ($EditionMode -eq 'Fixed') {
        ...
        return @(Get-WinWSSelectedInstallImage -InstallWim $InstallWim -EditionName $EditionName)
    }
...
function Invoke-WinWSIsoPipeline {
    [CmdletBinding()]
    param(
    ...
    $workDir = Join-Path (Get-Win11IsoProcessTempPath) ('WinWS_ISO_' + [Guid]::NewGuid().ToString('n'))
    $isoContents = Join-Path $workDir 'iso'
    $mountDir = Join-Path $workDir 'mount'
    $outputIso = Join-Path (Get-WinWSOutputDirectory) ('WinMint-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.iso')
    $mountedImage = $false
    ...
    try {
        ...
        if (Get-Command Write-WinWSHeadlessWorkMarker -ErrorAction SilentlyContinue) {
            Write-WinWSHeadlessWorkMarker -WorkDir $workDir -MountDir $mountDir -IsoContents $isoContents
        }
        if (Get-Command Set-WinWSHeadlessJournalPhase -ErrorAction SilentlyContinue) {
            Set-WinWSHeadlessJournalPhase -Phase 'StageIso' -WorkDir $workDir -MountDir $mountDir -IsoContents $isoContents
        }
        Start-PipelinePhase 'Stage ISO'
        $usedStageCache = $false
        $stageCacheDir = Get-WinWSIsoStageCacheHit -SourceIsoPath $BuildConfig.SourceIso
        if ($null -ne $stageCacheDir) {
            ...
            if (-not $DryRun) { Clear-WinWSReadOnlyAttribute -Path $isoContents }
        }
        else {
            Mount-WinWSIsoToWorkTree -SourceIso $BuildConfig.SourceIso -IsoContents $isoContents -DryRun:$DryRun
            $null = Convert-WinWSInstallEsdToWim -IsoContentsRoot $isoContents -DryRun:$DryRun
        }
        $installWim = Join-Path $isoContents 'sources\install.wim'
        if ($null -ne $script:WinWSBuildManifest) {
            if (Test-Path -LiteralPath $BuildConfig.SourceIso) {
                $script:WinWSBuildManifest.sizeDelta.sourceIsoBytes = (Get-Item -LiteralPath $BuildConfig.SourceIso).Length
            }
            if (Test-Path -LiteralPath $installWim) {
                $script:WinWSBuildManifest.sizeDelta.installWimBeforeServicingBytes = (Get-Item -LiteralPath $installWim).Length
            }
        }
        ...
        Test-RemoteBuildPrerequisite `
            ...
            -IsoContentsRoot $isoContents `
            -AutounattendPath (Join-Path $root 'config\autounattend.xml') `
            -ExportHostDriversRequested:$ExportHostDrivers
        Complete-PipelinePhase 'Stage ISO'
        if (-not $DryRun -and -not $usedStageCache) {
            Publish-WinWSIsoStageCache -SourceIsoPath $BuildConfig.SourceIso -IsoContentsPath $isoContents
        }
        ...
        $installImages = @(Get-WinWSInstallImagesForBuild -InstallWim $installWim -EditionMode $editionMode -EditionName $BuildConfig.Edition)
        ...
        if ($null -ne $script:WinWSBuildManifest) {
            ...
            $script:WinWSBuildManifest.source.editions = $editionNames
        }

        if ($DryRun) {
            $dryRunImage = $installImages | Select-Object -First 1
            $setupProfile = New-WinWSSetupProfile -BuildConfig $BuildConfig
            $agentProfile = New-WinWSAgentProfile -BuildConfig $BuildConfig
            $setupPlan = New-WinWSSetupPlan -BuildConfig $BuildConfig -SetupProfile $setupProfile -AgentProfile $agentProfile
            $preparedSetup = Install-Autounattend `
                -MountDir $mountDir `
                -IsoContents $isoContents `
                -AutounattendTemplate (Get-Content -LiteralPath (Join-Path $root 'config\autounattend.xml') -Raw) `
                -ImageArch $imageArch `
                ...
                -SetupPlan $setupPlan `
                -DryRun
            $dryRunArtifacts = Save-WinWSDryRunArtifacts `
                -Config $BuildConfig `
            ...
            Write-WinWSDryRunArtifactSummary -Artifacts $dryRunArtifacts
        ...
        Sync-NerdFont -FontDir (Join-Path $root 'assets\fonts')
        Sync-Cursor -CursorsDir (Join-Path $root 'assets\cursors') -PackKind $BuildConfig.CursorPackKind
        $localDriverRoot = Join-Path $root "assets\drivers\$imageArch"
        if (-not (Test-Path -LiteralPath $localDriverRoot)) { $localDriverRoot = Join-Path $root 'assets\drivers' }
        $expandedLocalMsis = @(Expand-DriverMsisUnderDriversFolder -DriverRoot $localDriverRoot -WorkDir $workDir -DryRun:$DryRun)

        $driverSources = [System.Collections.Generic.List[object]]::new()
        # Only add the repo assets folder when it contains real .inf files —
        # MSI-only folders are extracted into $workDir and added below instead.
        if ((Test-Path -LiteralPath $localDriverRoot -PathType Container) -and
            (Get-ChildItem -LiteralPath $localDriverRoot -Recurse -Filter '*.inf' -File -ErrorAction SilentlyContinue | Select-Object -First 1)) {
            $driverSources.Add([pscustomobject]@{ Source = $localDriverRoot; Label = 'Local assets' })
        }
        foreach ($p in $expandedLocalMsis) {
            $driverSources.Add([pscustomobject]@{ Source = $p; Label = "Local MSI ($([IO.Path]::GetFileName($p)))" })
        }
        ...
        if ($ExportHostDrivers) {
            ...
            $hostDriverCache = Get-WinWSHostDriverExportCacheHit
            if ($null -ne $hostDriverCache) {
                ...
                Clear-WinWSReadOnlyAttribute -Path $hostDriverDir
            }
            else {
                Invoke-Action 'Exporting host drivers for injection' {
                    Export-WinWSHostDrivers -Destination $hostDriverDir | Out-Null
                }
                Publish-WinWSHostDriverExportCache -SourceDir $hostDriverDir
            }
        ...
        if (Get-Command Set-WinWSHeadlessJournalPhase -ErrorAction SilentlyContinue) {
            Set-WinWSHeadlessJournalPhase -Phase 'ServiceWim' -WorkDir $workDir -MountDir $mountDir -IsoContents $isoContents
        }
        ...
        foreach ($image in $installImages) {
            ...
            $mountTimer.Stop()
            LogOK "install.wim index $($image.ImageIndex) mounted in $(Format-WinWSDuration -Duration $mountTimer.Elapsed); proceeding to servicing."
            $mountedImage = $true
            $setupProfile = New-WinWSSetupProfile -BuildConfig $BuildConfig
            $agentProfile = New-WinWSAgentProfile -BuildConfig $BuildConfig
            $setupPlan = New-WinWSSetupPlan -BuildConfig $BuildConfig -SetupProfile $setupProfile -AgentProfile $agentProfile
            ...
            Remove-WinWSCapabilities -MountDir $mountDir
            ...
            Remove-WinWSOneDriveSetupStub -MountDir $mountDir
            Enable-WinWSOptionalFeature -MountDir $mountDir -Features $BuildConfig.Features
        ...
        if ($null -ne $script:WinWSBuildManifest -and (Test-Path -LiteralPath $installWim)) {
            $script:WinWSBuildManifest.sizeDelta.installWimAfterServicingBytes = (Get-Item -LiteralPath $installWim).Length
        }
        ...
        if ($null -ne $script:WinWSBuildManifest) {
            ...
            $script:WinWSBuildManifest.drivers.injectedCount = $infNames.Count
            $script:WinWSBuildManifest.drivers.infNames = @($infNames | Sort-Object -Unique)
        }
        ...
        if ($null -ne $script:WinWSBuildManifest -and (Test-Path -LiteralPath $installWim)) {
            $script:WinWSBuildManifest.sizeDelta.installWimAfterExportBytes = (Get-Item -LiteralPath $installWim).Length
        }
        Start-PipelinePhase 'Assemble ISO'
        if (Get-Command Set-WinWSHeadlessJournalPhase -ErrorAction SilentlyContinue) {
            Set-WinWSHeadlessJournalPhase -Phase 'AssembleIso' -WorkDir $workDir -MountDir $mountDir -IsoContents $isoContents
        }
        New-WinWSIsoImage -IsoContents $isoContents -ImageArch $imageArch -OutputIso $outputIso
        if (Test-Path -LiteralPath $outputIso) {
            ...
            if ($null -ne $script:WinWSBuildManifest) {
                $script:WinWSBuildManifest.sizeDelta.outputIsoBytes = $isoItem.Length
                $sourceBytes = $script:WinWSBuildManifest.sizeDelta.sourceIsoBytes
                if ([long]$sourceBytes -gt 0) {
                    $script:WinWSBuildManifest.sizeDelta.outputMinusSourceBytes = [long]$isoItem.Length - [long]$sourceBytes
                    $script:WinWSBuildManifest.sizeDelta.outputToSourceRatio = [math]::Round(([double]$isoItem.Length / [double]$sourceBytes), 4)
                }
            }
            LogOK "Final ISO: $outputIso"
            LogOK "Final ISO size: $(Format-WinWSByteSize -Bytes $isoItem.Length) ($($isoItem.Length) bytes)."
        ...
        if (Get-Command Set-WinWSHeadlessJournalPhase -ErrorAction SilentlyContinue) {
            Set-WinWSHeadlessJournalPhase -Phase 'Cleanup' -WorkDir $workDir -MountDir $mountDir -IsoContents $isoContents
        }
