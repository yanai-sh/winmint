...
$script:Win11IsoBuildShortName = 'Windows Workstation Slim'
$script:Win11IsoVolumeLabel = 'WWKS_SLIM'
...
# ``Compact`` = same font for ``WinWS`` plus two caption lines for narrow viewports.
$script:Win11IsoHeroBannerNormal = @'
        +-+-+-+-+-+ +-+-+-+-+
        |W|i|n|W|S| |S|l|i|m|
        +-+-+-+-+-+ +-+-+-+-+

      Windows Workstation Slim
'@
...
$script:AppxBloatwareCategories = Get-WinWSAppxBloatwareCategories
...
# Cursor pack: fixed WinWS default, bundled with the repo.
$script:Win11IsoCursorPackCatalog = @{
    'BreezeXLight'        = @{
        ProjectPage         = 'https://github.com/ful1e5/BreezeX_Cursor/releases'
        PinnedReleaseLabel  = 'v2.0.1'
        HostSourceDir       = 'BreezeXLight'
        SchemeName          = 'BreezeX Light'
        DestSegment         = 'BreezeXLight'
        MarkerRelPath       = 'BreezeXLight\Arrow.cur'
        ExtractedSourceDirs = @()
        ExpectedSha256      = ''
        Bundled             = $true
    }
...
$script:Win11IsoDefaultCursorPackKind = 'BreezeXLight'
...
$script:Win11IsoCursorRegistryPairs = @(
    ...
    @{ Name = 'precisionhair'; File = 'Cross.cur' }
...
$script:Win11IsoCursorOptionalRegistryPairs = @(
    ...
    @{ Name = 'Pan'; File = 'Pan.cur' }
    @{ Name = 'Grabbing'; File = 'Grabbing.cur' }
    @{ Name = 'Zoom-in'; File = 'ZoomIn.cur' }
    @{ Name = 'Zoom-out'; File = 'ZoomOut.cur' }
)
