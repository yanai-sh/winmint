...
function Expand-DriverMsisUnderDriversFolder {
    <# <summary>If .\assets\drivers contains only .msi files, admin-install each into a temp
    subfolder under $WorkDir (NOT into the repo) so DISM can see the .inf files. Returns the
    extracted destination paths. The repo's assets\drivers\ tree is left untouched.</summary> #>
    param(
        [Parameter(Mandatory)][string]$DriverRoot,
        [Parameter(Mandatory)][string]$WorkDir,
        [switch]$DryRun
    )
    if (-not (Test-Path -LiteralPath $DriverRoot)) { return @() }

    $infCount = (Get-ChildItem -LiteralPath $DriverRoot -Recurse -Filter '*.inf' -File -ErrorAction SilentlyContinue | Measure-Object).Count
    if ($infCount -ge 1) { return @() }

    $msis = @(Get-ChildItem -LiteralPath $DriverRoot -Recurse -Filter '*.msi' -File -ErrorAction SilentlyContinue)
    if ($msis.Count -eq 0) { return @() }

    $extractRoot = Join-Path $WorkDir 'local_driver_msi'
    $expanded = [System.Collections.Generic.List[string]]::new()
    if ($DryRun) {
        $null = New-Item -ItemType Directory -Path $extractRoot -Force
        Log "Expanding $($msis.Count) driver MSI(s) from .\assets\drivers into the temp work folder so DISM can see .inf files…"
        foreach ($msi in $msis) {
            $base = [IO.Path]::GetFileNameWithoutExtension($msi.Name)
            if ([string]::IsNullOrWhiteSpace($base)) { $base = 'driver' }
            $safe = $base -creplace '[^\w\-\.]', '_'
            $dest = Join-Path $extractRoot $safe
            LogDry "Would administrative-install driver MSI: $($msi.Name)"
            LogVerbose "msiexec /a `"$($msi.FullName)`" /qn TARGETDIR=`"$dest`""
        }
        return $expanded.ToArray()
    }

    $fingerprint = Get-WinWSDriverMsiSetFingerprint -MsiFiles $msis
    $cachedBundle = Get-WinWSDriverMsiBundleCacheHit -Fingerprint $fingerprint
    if ($null -ne $cachedBundle) {
        Log 'Restoring driver MSI administrative extracts from temp cache…'
        $null = New-Item -ItemType Directory -Path $extractRoot -Force
        Invoke-RobocopyChecked -Source $cachedBundle -Dest $extractRoot -UserFacingMessage 'Copying cached driver MSI extracts into the work folder…'
        Clear-WinWSReadOnlyAttribute -Path $extractRoot
        foreach ($msi in $msis) {
            $base = [IO.Path]::GetFileNameWithoutExtension($msi.Name)
            if ([string]::IsNullOrWhiteSpace($base)) { $base = 'driver' }
            $safe = $base -creplace '[^\w\-\.]', '_'
            $expanded.Add((Join-Path $extractRoot $safe))
        }
        return $expanded.ToArray()
    }
    ...
    foreach ($msi in $msis) {
        ...
        $safe = $base -creplace '[^\w\-\.]', '_'
        $dest = Join-Path $extractRoot $safe
        $added = Invoke-DriverMsiAdministrativeInstall -MsiPath $msi.FullName -Destination $dest
        LogOK "Expanded $($msi.Name) ($added .inf file(s))."
        LogVerbose $dest
        $expanded.Add($dest)
    }
    Publish-WinWSDriverMsiBundleCache -Fingerprint $fingerprint -SourceParentDir $extractRoot
    return $expanded.ToArray()
}
...
function Expand-DriverMSI {
    ...
    $cached = Get-WinWSDriverMsiSingleExtractCacheHit -MsiPath $MsiPath
    if ($null -ne $cached) {
        ...
        Clear-WinWSReadOnlyAttribute -Path $Destination
        return
    }
    ...
    Publish-WinWSDriverMsiSingleExtractCache -MsiPath $MsiPath -SourceDir $Destination
}

function Expand-WinWSDriverZip {
    param(
...
function Resolve-Win11IsoCustomDriverSource {
    ...
    if ($item.PSIsContainer) {
        ...
        if ($null -ne $cachedBundle) {
            ...
            Clear-WinWSReadOnlyAttribute -Path $dest
        ...
        Publish-WinWSDriverMsiBundleCache -Fingerprint $fingerprint -SourceParentDir $dest
        return [pscustomobject]@{ Source = $dest; Label = 'Custom drivers (expanded MSI)'; Ready = $true }
    }
    ...
    switch ($item.Extension.ToLowerInvariant()) {
        ...
        '.zip' {
            ...
            Expand-WinWSDriverZip -ZipPath $item.FullName -Destination $dest
...
function Export-WinWSHostDrivers {
...
function Get-WinWSInfClassName {
...
function Copy-WinWSSetupCriticalDrivers {
    param(
    ...
    foreach ($inf in Get-ChildItem -LiteralPath $DriverSource -Recurse -Filter '*.inf' -File -ErrorAction SilentlyContinue) {
        $class = Get-WinWSInfClassName -InfPath $inf.FullName
...
function Invoke-DriverInjection {
    ...
    Invoke-Action 'Injecting drivers into Windows and WinPE (when boot.wim is present)' {
        ...
        $windowsTimer.Stop()
        LogOK "Windows image driver injection finished in $(Format-WinWSDuration -Duration $windowsTimer.Elapsed)."
        ...
        if (Test-Path $bootWim) {
            ...
            try {
                $bootInfCount = Copy-WinWSSetupCriticalDrivers -DriverSource $DriverSource -Destination $bootDriverSource
                ...
                foreach ($idx in $bootIndexes) {
                    ...
                    try {
                        ...
                        $bootTimer.Stop()
                        LogOK "boot.wim index $idx driver injection saved in $(Format-WinWSDuration -Duration $bootTimer.Elapsed)."
                    }
                    catch {
                        $bootTimer.Stop()
