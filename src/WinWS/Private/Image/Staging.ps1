...
function Clear-WinWSReadOnlyAttribute {
...
function Test-RemoteBuildPrerequisite {
    ...
    $gh = @{
        'User-Agent'      = 'WinWS/1.0 (PowerShell offline ISO builder)'
    ...
    if (-not (Test-WinWSGitHubApiReachable -TimeoutSec 5)) {
    ...
    Start-Sleep -Milliseconds 150
    $viveResolved = Get-WinWSViveToolReleaseAsset -TargetArch $TargetArch -Headers $gh
...
function Remove-WinWSCapabilities {
    ...
    if ($null -ne $script:WinWSBuildManifest) {
        $script:WinWSBuildManifest.removals.capabilitiesRemoved = $removed.ToArray()
    }
...
function Remove-WinWSOneDriveSetupStub {
    param([ValidateNotNullOrEmpty()][string]$MountDir)
    ...
    if ($null -ne $script:WinWSBuildManifest) {
        $script:WinWSBuildManifest.removals['oneDriveSetupStubs'] = [ordered]@{
...
function Invoke-AppxRemoval {
    ...
    Invoke-Action 'Removing optional preinstalled Store apps from the image' {
        ...
        if ($null -ne $script:WinWSBuildManifest) {
            $script:WinWSBuildManifest.removals.appxRemoved = $removedNames.ToArray()
            $script:WinWSBuildManifest.removals.appxRemovedCount = $removed
        }
