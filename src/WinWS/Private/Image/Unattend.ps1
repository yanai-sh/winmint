...
function New-WinWSAgentProfile {
    ...
    $profileGroups = @($BuildConfig.ProfileGroups)
    $needsFlowEverything = (
        $profileGroups -contains 'Developer' -or
        $profileGroups -contains 'DesktopUI'
    )
    if ($needsFlowEverything) { $needsPackageManagers = $true }
    [ordered]@{
        ...
        modules = [ordered]@{
            ...
            # Primary launcher + instant file search for Developer/DesktopUI profiles only.
            flowEverything = [ordered]@{ enabled = $needsFlowEverything }
            liveInstallAudit = [ordered]@{ enabled = $true }
            phoneLink = [ordered]@{
                enabled = $true
                showInFileExplorer = $true
                crossDeviceCopyPaste = $true
                hideCrossDeviceHomeFolder = $true
            }
            shell = [ordered]@{
...
function New-WinWSSetupProfile {
    param([Parameter(Mandatory)]$BuildConfig)
...
function New-WinWSSetupPlan {
    ...
    [ordered]@{
        ...
        phases = @(
            ...
            [ordered]@{
                ...
                responsibilities = @(
                    ...
                    'load setup profile from WinWSSetupProfile.json'
                )
            }
            [ordered]@{
                id = 'setupComplete'
        ...
        stagedArtifacts = @(
            ...
            'C:\Windows\Setup\Scripts\WinWSAgent\BuildProfile.json'
        )
        ...
        notes = @(
            ...
            'Windows setup uses Germany as the EEA region to bake DMA interoperability, then FirstLogon restores the builder regional defaults.',
            'OneDrive is not offered or auto-provisioned by default; manual reinstall remains possible after setup.'
        )
    }
}

function Get-WinWSDualBootWindowsRatio {
    param([string]$Preset)
...
function New-WinWSWindowsOnlyDiskpartPowerShellCommand {
    param([Parameter(Mandatory)]$DiskLayout)

    $windowsMinimumGb = [int](Get-WinWSProfileSetting $DiskLayout 'windowsMinimumGb' 256)
    $efiMb = [int](Get-WinWSProfileSetting $DiskLayout 'efiMb' 1024)
    $msrMb = [int](Get-WinWSProfileSetting $DiskLayout 'msrMb' 16)
    $recoveryMb = [int](Get-WinWSProfileSetting $DiskLayout 'recoveryMb' 1024)
...
'format quick fs=ntfs label=WINWS',
'shrink minimum=$recoveryMb desired=$recoveryMb',
'create partition primary size=$recoveryMb',
'format quick fs=ntfs label=WinRE',
'set id=de94bba4-06d1-4d40-a16a-bfd50179d6ac',
'gpt attributes=0x8000000000000001'
) | Set-Content -LiteralPath 'X:\winws_dp.txt' -Encoding ASCII;
diskpart.exe /s X:\winws_dp.txt
"@
    $encoded = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($script))
    "powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand $encoded"
}

function New-WinWSDualBootDiskpartPowerShellCommand {
    param([Parameter(Mandatory)]$DiskLayout)

    $preset = [string](Get-WinWSProfileSetting $DiskLayout 'preset' '')
    $ratio = Get-WinWSDualBootWindowsRatio -Preset $preset
    $roundingGb = [int](Get-WinWSProfileSetting $DiskLayout 'roundingGb' 64)
    $windowsMinimumGb = [int](Get-WinWSProfileSetting $DiskLayout 'windowsMinimumGb' 256)
    $linuxMinimumGb = [int](Get-WinWSProfileSetting $DiskLayout 'linuxMinimumGb' 128)
    $efiMb = [int](Get-WinWSProfileSetting $DiskLayout 'efiMb' 1024)
    $msrMb = [int](Get-WinWSProfileSetting $DiskLayout 'msrMb' 16)
    $recoveryMb = [int](Get-WinWSProfileSetting $DiskLayout 'recoveryMb' 1024)
...
function Install-Autounattend {
    ...
    if ($SetupPlan -and $null -ne $script:WinWSBuildManifest) {
        $script:WinWSBuildManifest['setupPlan'] = [ordered]@{
    ...
    if ($EditionMode -eq 'Fixed') {
        ...
        Log "Edition mode: fixed image selection ($EditionName)."
    }
    ...
    $diskLayoutMode = if ($DiskLayout) { [string](Get-WinWSProfileSetting $DiskLayout 'mode' '') } else { '' }
    ...
    if (-not $AutoWipeDisk) {
        ...
        if ($diskLayoutMode -eq 'DualBootReserved') {
            $preset = [string](Get-WinWSProfileSetting $DiskLayout 'preset' 'Balanced')
        ...
        if ($wpSetupComp) {
            ...
            $dpLines = if ($diskLayoutMode -eq 'DualBootReserved') {
                @(New-WinWSDualBootDiskpartPowerShellCommand -DiskLayout $DiskLayout)
            }
            else {
                @(New-WinWSWindowsOnlyDiskpartPowerShellCommand -DiskLayout $DiskLayout)
            }
    ...
    if ($DryRun) {
        ...
        if ($AgentProfile) {
            ...
            LogOK 'Validated WinWSAgent profile JSON generation in memory.'
        }
        $setupPlanJson = ''
        if ($SetupPlan) {
    ...
    Invoke-Action 'Writing autounattend.xml and setup scripts onto the ISO' {
        ...
        if (Test-Path -LiteralPath $bundleDir) {
            ...
            $wallpaperSrc = Join-Path $ScriptRoot 'assets\Bloom-wallpaper-OLED-muted.png'
            if (-not (Test-Path -LiteralPath $wallpaperSrc)) {
                throw "WinWS wallpaper asset is missing: $wallpaperSrc"
            }
            $wallpaperDir = Join-Path $MountDir 'Windows\Web\Wallpaper\WinWS'
            $null = New-Item -ItemType Directory -Path $wallpaperDir -Force -ErrorAction SilentlyContinue
            Copy-Item -LiteralPath $wallpaperSrc -Destination (Join-Path $wallpaperDir 'WinWS-Bloom-OLED.png') -Force
            LogOK 'Staged WinWS Bloom OLED wallpaper into the offline image.'
            $utf8Bom = [System.Text.UTF8Encoding]::new($true)
            foreach ($n in @('SetupComplete.cmd', 'SetupComplete.ps1', 'Specialize.ps1', 'DefaultUser.ps1', 'FirstLogon.ps1', 'Maintain.ps1', 'Audit-LiveInstall.ps1')) {
            ...
            if ($SetupProfile) {
                $setupProfileJson = $SetupProfile | ConvertTo-Json -Depth 12
                Set-Content -LiteralPath (Join-Path $destScripts 'WinWSSetupProfile.json') -Value $setupProfileJson -Encoding UTF8
                LogOK 'Generated setup profile for specialize, SetupComplete, and FirstLogon scripts.'
            }
            if ($SetupPlan) {
                $setupPlanJson = $SetupPlan | ConvertTo-Json -Depth 16
                Set-Content -LiteralPath (Join-Path $destScripts 'WinWSSetupPlan.json') -Value $setupPlanJson -Encoding UTF8
                LogOK 'Generated setup plan for CLI/UI inspection and install-phase audit.'
            }
            $agentSrc = Join-Path $ScriptRoot 'src\WinWS.Agent'
            if (Test-Path -LiteralPath $agentSrc) {
                $agentDest = Join-Path $destScripts 'WinWSAgent'
                Copy-Item -LiteralPath $agentSrc -Destination $agentDest -Recurse -Force
                $pkgManifest = Join-Path $ScriptRoot 'config\packages.json'
                ...
                if ($AgentProfile) {
                    ...
                    LogOK 'Generated WinWSAgent profile from the selected wizard options.'
                }
                ...
                if ($windhawkSelected) {
                    ...
                    Copy-Item -LiteralPath (Join-Path $ScriptRoot 'scripts\setup\WindhawkBootstrap.ps1') -Destination (Join-Path $windhawkAssetDir 'WindhawkBootstrap.ps1') -Force
                    Copy-Item -LiteralPath (Join-Path $ScriptRoot 'scripts\setup\WindhawkBootstrap.Helpers.ps1') -Destination (Join-Path $windhawkAssetDir 'WindhawkBootstrap.Helpers.ps1') -Force
                    Copy-Item -LiteralPath (Join-Path $ScriptRoot 'scripts\setup\DisableVirtualDesktopFlyouts.ps1') -Destination (Join-Path $windhawkAssetDir 'DisableVirtualDesktopFlyouts.ps1') -Force
                    Copy-Item -LiteralPath (Join-Path $ScriptRoot 'assets\windhawk\preset.json') -Destination (Join-Path $windhawkAssetDir 'preset.json') -Force
                ...
                if ($yasbSelected) {
                    $yasbSourceDir = Join-Path $ScriptRoot 'assets\yasb'
                ...
                if ($komorebiSelected) {
                    $komorebiSourceDir = Join-Path $ScriptRoot 'assets\komorebi'
