...
function Sync-NerdFont {
    ...
    Invoke-Action 'Downloading Cascadia Code (Nerd Font) into .\assets\fonts if missing' {
        ...
        try {
            ...
            $cachedRoot = Get-WinWSCascadiaNerdFontCacheHit
            if ($null -ne $cachedRoot) {
                Log 'Restoring Cascadia Code Nerd Font from temp cache (skipping GitHub + zip extract)…'
                Restore-WinWSCascadiaNerdFontFromCache -FontDir $FontDir -CacheRootDir $cachedRoot
            ...
            Add-WinWSManifestPayload -Name 'Cascadia Code (Nerd Font)' -SourceUrl $cascZipUrl `
            ...
            try {
                ...
                Publish-WinWSCascadiaNerdFontCache -FontDir $FontDir -TagName $cascRel.tag_name -AssetName $cascAsset.name `
...
function Sync-Cursor {
    ...
    if (-not $script:Win11IsoCursorPackCatalog.ContainsKey($PackKind)) {
        throw "Unsupported cursor pack '$PackKind'. WinWS uses BreezeX Light."
    }
    ...
    Invoke-Action "Checking bundled cursor pack ($PackKind)" {
        ...
        $missingRequired = @(
            $script:Win11IsoCursorSchemeOrder |
                Where-Object { -not (Test-Path -LiteralPath (Join-Path $srcDir $_)) }
        )
        if ($missingRequired.Count -gt 0) {
            throw "Bundled cursor pack '$PackKind' is missing required role file(s): $($missingRequired -join ', ')"
        }
        LogOK "Bundled BreezeX Light cursor pack is present."
    }
}
...
function Install-OfflineCursor {
    param(
        ...
        [string]$CursorPackKind = 'BreezeXLight'
    )
    ...
    if (-not $script:Win11IsoCursorPackCatalog.ContainsKey($CursorPackKind)) {
        throw "Unsupported cursor pack '$CursorPackKind'. WinWS uses BreezeX Light."
    }
    ...
    Invoke-Action "Installing cursor scheme ($CursorPackKind) for the default user profile" {
        ...
        $cursorFiles | Copy-Item -Destination $destDir -Force
        ...
        $regPairs += @(
            $script:Win11IsoCursorOptionalRegistryPairs |
                Where-Object { Test-Path -LiteralPath (Join-Path $srcDir $_.File) }
        )
        ...
        try {
            ...
            $null = & reg.exe add "`"$schemesKey`"" /v "$schemeName" /t REG_SZ /d "$schemeList" /f
            ...
            $null = & reg.exe add 'HKLM\peNTUSER\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects\CursorShadow' /v DefaultValue /t REG_DWORD /d 1 /f
        }
        finally {
            [GC]::Collect(); [GC]::WaitForPendingFinalizers(); Start-Sleep -Milliseconds 500
    ...
    Invoke-Action 'Provisioning winget with DISM (offline msixbundle)' {
        ...
        try {
            ...
            try {
                ...
                $bundlePath = Get-WinWSCachedDownloadFile -Patterns @('Microsoft.DesktopAppInstaller_*.msixbundle', '*.msixbundle')
            ...
            Add-WinWSManifestPayload -Name 'winget' -SourceUrl $sourceUrl `
