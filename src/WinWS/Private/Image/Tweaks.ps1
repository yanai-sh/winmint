        ...
        try {
            ...
            foreach ($group in $script:RegistryTweaks) {
                if ($selected.Count -gt 0 -and $selected -notcontains [string]$group.id) {
                    ...
                    if (Get-Command Add-WinWSManifestRegistryTweakEvent -ErrorAction SilentlyContinue) {
                        Add-WinWSManifestRegistryTweakEvent -Group $group -Status 'skipped-not-selected'
                    }
                    continue
                }
                ...
                if ($null -ne $conditional -and -not (& $conditional)) {
                    ...
                    if (Get-Command Add-WinWSManifestRegistryTweakEvent -ErrorAction SilentlyContinue) {
                        Add-WinWSManifestRegistryTweakEvent -Group $group -Status 'skipped-conditional'
                    }
                    continue
                }
                ...
                try {
                    ...
                    if (Get-Command Add-WinWSManifestRegistryTweakEvent -ErrorAction SilentlyContinue) {
                        Add-WinWSManifestRegistryTweakEvent -Group $group -Status 'applied'
                    }
                }
                catch {
                    if (Get-Command Add-WinWSManifestRegistryTweakEvent -ErrorAction SilentlyContinue) {
                        Add-WinWSManifestRegistryTweakEvent -Group $group -Status 'failed' -ErrorMessage $_.Exception.Message
                    }
                    throw
                }
            }
...
function Enable-WinWSOptionalFeature {
    param(
        [ValidateNotNullOrEmpty()][string]$MountDir,
        [string[]]$Features = @()
    )
...
function Get-HostUnattendRegionalDefault {
    ...
    $homeLocationGeoId = Resolve-WinWSRegionGeoId -CultureName $userLocaleName
...
function Invoke-GoldImageRegionalPrompt {
    ...
    return [pscustomobject]@{
        ...
        HomeLocationGeoId    = Resolve-WinWSRegionGeoId -CultureName $cultureName
    }
}
...
function Merge-UnattendInternationalXml {
    ...
    $spec = $XmlDoc.SelectSingleNode('//u:settings[@pass="specialize"]/u:component[@name="Microsoft-Windows-International-Core"]', $NsMgr)
    if ($spec) {
        foreach ($pair in @(
                @{ El = 'InputLocale'; V = $InputLocale }
                @{ El = 'SystemLocale'; V = $SystemLocale }
            ...
            if ([string]::IsNullOrWhiteSpace($pair.V)) { continue }
            $xn = $spec.SelectSingleNode("u:$($pair.El)", $NsMgr)
            if ($null -ne $xn) { $xn.InnerText = $pair.V }
        }
    }
    else {
        LogWarn 'autounattend.xml: no specialize International-Core block (locale merge may be incomplete).'
    }
}
