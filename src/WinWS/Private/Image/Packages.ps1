...
function Copy-WinWSPayloadDirectoryChildren {
    param(
    ...
    Invoke-Action 'Installing PowerShell 7 into the offline image (pwsh for setup scripts)' {
        ...
        try {
            ...
            try {
                ...
                $rel = Invoke-RestMethod -Verbose:$false -Uri 'https://api.github.com/repos/PowerShell/PowerShell/releases/latest' -Headers @{ 'User-Agent' = 'WinWS/1.0' }
                ...
                $zip = Invoke-WebRequestCachedFile -Uri $asset.browser_download_url -CacheFileName $asset.name -Headers @{ 'User-Agent' = 'WinWS/1.0' }
                ...
                $zip = Get-WinWSCachedDownloadFile -Patterns @("PowerShell-*-$suffix.zip")
            ...
            Add-WinWSManifestPayload -Name 'PowerShell 7' -SourceUrl $sourceUrl `
            ...
            Copy-WinWSPayloadDirectoryChildren -SourceDir $sourceDir -DestinationDir $dest
...
function Get-WinWSViveToolReleaseAsset {
    param(
        [Parameter(Mandatory)][string]$TargetArch,
        [hashtable]$Headers = @{ 'User-Agent' = 'WinWS/1.0' }
    )
...
function Install-OfflineViveTool {
    ...
    Invoke-Action 'Staging ViVeTool for SetupComplete feature overrides' {
        ...
        try {
            ...
            $version = 'cached'
            try {
                $resolved = Get-WinWSViveToolReleaseAsset -TargetArch $TargetArch -Headers $headers
                ...
                $zip = Get-WinWSCachedDownloadFile -Patterns @(Get-WinWSViveToolCachePattern -Architecture $TargetArch)
            ...
            Add-WinWSManifestPayload -Name 'ViVeTool' -SourceUrl $sourceUrl `
            ...
            if (Test-Path -LiteralPath $dest) { Remove-Item -LiteralPath $dest -Recurse -Force -ErrorAction Stop }
            Copy-WinWSPayloadDirectoryChildren -SourceDir $exe.DirectoryName -DestinationDir $dest
...
function Remove-NonEnglishLanguageFeature {
    ...
    Invoke-Action 'Removing non-English language feature packages from the image' {
        ...
        if ($null -ne $script:WinWSBuildManifest) {
            $script:WinWSBuildManifest.removals.languagePackagesRemoved = $removedPackages.ToArray()
            $script:WinWSBuildManifest.removals.languagePackagesRemovedCount = $removedPackages.Count
        }
    }
}
...
function Save-ImageWithCleanup {
    ...
    Invoke-Action 'Running DISM component cleanup on the mounted Windows image' -SpectreProgressIndeterminate {
        LogVerbose "Mount: $MountDir"
        if ($null -ne $script:WinWSBuildManifest) {
            $script:WinWSBuildManifest.servicing.componentCleanup = 'StartComponentCleanup'
            $script:WinWSBuildManifest.servicing.resetBase = $false
            $script:WinWSBuildManifest.servicing.serviceabilityPolicy = 'Preserve component-store uninstall/repair metadata; do not run ResetBase by default.'
        }
...
function Resolve-OscdimgPath {
    ...
    if (-not $oscdimg) {
        throw @(
            ...
            'WinWS tries Microsoft.OSCDIMG first, then Microsoft.WindowsADK latest, then version 10.1.26100.2454.'
        ) -join ' '
    }
    return $oscdimg
}
