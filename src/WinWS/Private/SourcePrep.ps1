...
function Get-WinWSUupPrepRoot {
    Join-Path (Get-WinWSOutputDirectory) '.uup'
}

function Get-WinWSUupZipFingerprint {
...
function Set-WinWSUupConvertConfigPolicy {
    param([Parameter(Mandatory)][string]$Path)
...
function Get-WinWSPreparedUupIso {
...
function Format-WinWSUupByteSize {
    param([long]$Bytes)
...
function Get-WinWSUupDirectoryByteCount {
    param([Parameter(Mandatory)][string]$Path)
...
function Get-WinWSUupNewestFile {
    param([Parameter(Mandatory)][string]$Path)
...
function Get-WinWSUupToolProcessNames {
    $names = @(
...
function Get-WinWSUupSourcePrepSnapshot {
    param(
    ...
    $aria2Script = @(Get-ChildItem -LiteralPath (Join-Path $WorkDir 'files') -Filter 'aria2_script*.txt' -File -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTimeUtc -Descending |
        Select-Object -First 1)
    $iso = Get-WinWSPreparedUupIso -WorkDir $WorkDir
    $uupBytes = Get-WinWSUupDirectoryByteCount -Path $uupsDir
    $newest = Get-WinWSUupNewestFile -Path $WorkDir
    $processes = @(Get-WinWSUupToolProcessNames)
...
function Write-WinWSUupProgressSnapshot {
    param(
...
function Format-WinWSUupProgressMessage {
    ...
    if ([long]$Snapshot.UupBytes -gt 0) {
        $parts.Add("payloads $(Format-WinWSUupByteSize -Bytes ([long]$Snapshot.UupBytes)) in $($Snapshot.UupFileCount) file(s)") | Out-Null
    }
    if ([long]$Snapshot.InstallWimBytes -gt 0) {
        $parts.Add("install.wim $(Format-WinWSUupByteSize -Bytes ([long]$Snapshot.InstallWimBytes))") | Out-Null
    }
...
function Wait-WinWSUupDumpProcess {
    param(
    ...
    while (-not $Process.HasExited) {
        $snapshot = Get-WinWSUupSourcePrepSnapshot -WorkDir $WorkDir -StartedAt $started
        Write-WinWSUupProgressSnapshot -Snapshot $snapshot -ProgressLogPath $ProgressLogPath
        ...
        if ($phaseChanged -or ($fileChanged -and $timeForHeartbeat) -or $timeForHeartbeat) {
            $message = Format-WinWSUupProgressMessage -Snapshot $snapshot
    ...
    $final = Get-WinWSUupSourcePrepSnapshot -WorkDir $WorkDir -StartedAt $started
    Write-WinWSUupProgressSnapshot -Snapshot $final -ProgressLogPath $ProgressLogPath
    if (-not [string]::IsNullOrWhiteSpace([string]$final.IsoPath)) {
        LogOK "$(Format-WinWSUupProgressMessage -Snapshot $final); ISO $([IO.Path]::GetFileName([string]$final.IsoPath))"
    }
}

function Test-WinWSUupDumpFolder {
    param([Parameter(Mandatory)][string]$Path)

    if (-not (Test-Path -LiteralPath $Path -PathType Container)) { return $false }
    return ((Test-Path -LiteralPath (Join-Path $Path 'uup_download_windows.cmd') -PathType Leaf) -and
            (Test-Path -LiteralPath (Join-Path $Path 'ConvertConfig.ini') -PathType Leaf) -and
            (Test-Path -LiteralPath (Join-Path $Path 'files\get_aria2.ps1') -PathType Leaf))
}

function Invoke-WinWSPreparedUupFolderSourcePrep {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$UupDumpFolder)

    if (-not (Test-WinWSUupDumpFolder -Path $UupDumpFolder)) {
        throw "UUP Dump folder must contain uup_download_windows.cmd, ConvertConfig.ini, and files\get_aria2.ps1: $UupDumpFolder"
    }

    $item = Get-Item -LiteralPath $UupDumpFolder -ErrorAction Stop
    $iso = Get-WinWSPreparedUupIso -WorkDir $item.FullName
    if ([string]::IsNullOrWhiteSpace($iso)) {
        throw "Prepared UUP Dump folder has no generated ISO. Run the UUP Dump conversion first, then retry: $($item.FullName)"
    }

    [pscustomobject]@{
        SourceKind = 'PreparedUupDumpFolder'
        UupDumpFolder = $item.FullName
        GeneratedIso = $iso
        Reused = $true
        RanConversion = $false
        Logs = Join-Path $item.FullName 'logs'
    }
}

function Invoke-WinWSUupDumpSourcePrep {
    [CmdletBinding()]
    param(
    ...
    if (-not (Test-WinWSUupDumpZip -Path $UupDumpZip)) {
    ...
    $fp = Get-WinWSUupZipFingerprint -Path $UupDumpZip
    $workDir = Join-Path (Get-WinWSUupPrepRoot) $fp.Key
    ...
    $existingIso = Get-WinWSPreparedUupIso -WorkDir $extractDir
    ...
    Set-WinWSUupConvertConfigPolicy -Path (Join-Path $extractDir 'ConvertConfig.ini')
    ...
    Set-WinWSHeadlessJournalPhase -Phase 'PrepareSource'
    ...
    Wait-WinWSUupDumpProcess -Process $proc -WorkDir $extractDir -ProgressLogPath $progressLog
    if ($proc.ExitCode -ne 0) {
        throw "UUP Dump conversion failed with exit code $($proc.ExitCode). Logs: $logDir"
    }

    $iso = Get-WinWSPreparedUupIso -WorkDir $extractDir
    if ([string]::IsNullOrWhiteSpace($iso)) {
