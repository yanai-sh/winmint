...
function Initialize-GpuiLabToolPath {
    foreach ($scope in @('Machine', 'User')) {
        $pathValue = [Environment]::GetEnvironmentVariable('Path', $scope)
        foreach ($entry in @($pathValue -split ';')) {
            Add-GpuiLabToolPath -ToolPath $entry
        }
    }

    Add-GpuiLabToolPath -ToolPath (Join-Path $env:USERPROFILE '.cargo\bin')
    Add-GpuiLabToolPath -ToolPath 'C:\Program Files\LLVM\bin'
}
