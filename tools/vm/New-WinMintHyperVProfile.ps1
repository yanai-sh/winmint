...
.EXAMPLE
    pwsh -NoProfile -File .\tools\vm\New-WinMintHyperVProfile.ps1 -Tier Smoke -OutPath .\output\hyper-v-smoke.json -SourceIso .\tests\fixtures\iso\official-win11-25h2-english-arm64-v2.iso
#>
[CmdletBinding()]
param(
...
Set-WinMintVmAcceptanceProfileMetadata -Profile $profile -Tier $Tier | Out-Null
$profile | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $OutPath -Encoding UTF8

& $pwsh -NoProfile -File (Join-Path $PSScriptRoot 'Test-WinMintHyperVProfile.ps1') -ProfilePath $OutPath -Tier $Tier
if ($LASTEXITCODE -ne 0) {
    throw "Hyper-V profile validation failed with exit code $LASTEXITCODE."
}

Write-Host "Hyper-V profile written ($Tier): $OutPath"
