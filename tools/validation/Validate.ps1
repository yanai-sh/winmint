...
$validationSteps = [ordered]@{
    ...
    'JSON schemas' = {
        ...
        Test-RuntimeStateSchema
        Test-DiagnosticsOverlaySchema
    ...
    'Optional release smoke' = {
        if ($RunReleaseSmoke) {
            ...
            & (Get-WinMintPath -Name ReleaseToolsRoot -ChildPath 'New-WinMintReleaseBundle.ps1') -Version $version
            ...
            Write-Host 'Skipping release smoke. Use -RunReleaseSmoke after building the release bundle.'
        }
    }
    'PowerShell parser' = {
        # Get-ValidationPowerShellFile recurses *.ps1 (excluded: .git\ output\ dist\).
