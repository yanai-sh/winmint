...
function Invoke-ReleaseSmokeGuiLaunch {
    ...
    $gui = Join-Path $ReleaseRoot 'apps\gui\bin\WinMint-GUI.exe'
    Write-ReleaseSmokeLog "Starting packaged GUI for manual launch smoke: $gui"
    $process = Start-Process -FilePath $gui -PassThru
    Write-ReleaseSmokeLog "Packaged GUI process started with PID $($process.Id)."
}
