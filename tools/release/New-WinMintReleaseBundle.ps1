...
function Assert-WinMintReleaseInputs {
    ...
    $wizardHost = Join-Path $root "assets\runtime\setup\setup-shell\bin\$archFolder\WinMintSetupShell.exe"
    if (-not (Test-Path -LiteralPath $wizardHost -PathType Leaf)) {
        throw "Packaged wizard host missing: $wizardHost"
    }

    $wizardAssets = @('wizard.html', 'wizard.js', 'wizard.css')
    foreach ($asset in $wizardAssets) {
        $path = Join-Path $root "assets\runtime\setup\setup-shell\$asset"
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Wizard asset missing: $path"
        }
    }

    $bootstrap = Get-Content -LiteralPath (Get-WinMintPath -Name RepoRoot -ChildPath 'winmint.ps1') -Raw
    if ($bootstrap -match '''Gui''\s*\{\s*Find-WinMintWizardLauncherScript\s+-Root\s+\$versionRoot\s*\}') {
...
try {
    $buildSetupShell = Get-WinMintPath -Name ReleaseToolsRoot -ChildPath 'Build-WinMintSetupShell.ps1'
    & $buildSetupShell -AllArch
    if ($LASTEXITCODE -ne 0) {
        throw "Setup shell build failed with exit code $LASTEXITCODE."
    }
