...
if (-not (Get-Command Get-WinMintPath -ErrorAction SilentlyContinue)) {
    ...
    while (-not [string]::IsNullOrWhiteSpace($candidateRoot)) {
        $corePath = Join-Path $candidateRoot 'src\engine\Core.ps1'
    ...
    if (-not (Get-Command Get-WinMintPath -ErrorAction SilentlyContinue)) {
        throw 'Could not locate src\engine\Core.ps1 from the legacy WPF app path.'
    }
}
...
function Set-WinMintUiImageAssets {
    ...
    $imageMap = [ordered]@{
        ShellIconWindhawk  = 'assets\ui\desktop\windhawk.png'
        ShellIconYasb      = 'assets\ui\desktop\yasb.png'
        ShellIconKomorebi  = 'assets\ui\desktop\komorebi.png'
    }
...
function Set-WinMintUiVectorIconAssets {
    ...
    $iconMap = [ordered]@{
        WslIconUbuntu      = 'assets\ui\wsl\ubuntu.svg'
        WslIconDebian      = 'assets\ui\wsl\debian.svg'
        WslIconArch        = 'assets\ui\wsl\archlinux.svg'
        WslIconFedora      = 'assets\ui\wsl\fedora.svg'
        EditorIconNeovim   = 'assets\ui\editors\neovim.svg'
        EditorIconVSCodium = 'assets\ui\editors\vscodium.svg'
        EditorIconCursor   = 'assets\ui\editors\cursor.svg'
        EditorIconZed      = 'assets\ui\editors\zedindustries.svg'
    }
...
function Start-WinMintUIApp {
    ...
    $uiRoot = Get-WinMintPath -Name LegacyUiApp
    foreach ($module in @(
