...
# Minimal logging used by src\engine\Private\Media.ps1 when the full engine is not loaded.
...
function Initialize-WinMintUiMountHygiene {
    ...
    $core = Join-Path $RepositoryRoot 'src\engine\Core.ps1'
    ...
    $self = Get-WinMintPath -Name LegacyUiApp -ChildPath 'Services\MountCleanup.ps1'
