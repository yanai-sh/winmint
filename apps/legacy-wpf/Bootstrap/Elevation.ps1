...
function Invoke-ElevateForBuild {
    ...
    $scriptPath = Join-Path $script:WinMintRepositoryRoot 'WinMint-LegacyUI.ps1'
    ...
    Start-Process -FilePath (Get-WinMintPwshPath) -ArgumentList $pwshArgs -Verb RunAs
}
...
function Invoke-SelfElevate {
    ...
    $pwshExe = Get-WinMintPwshPath
    $waitForChild = $env:WINMINT_UI_WAIT_FOR_ELEVATED_CHILD -eq '1'
    if ($waitForChild) {
