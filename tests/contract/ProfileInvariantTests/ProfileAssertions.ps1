...
function Assert-FormFactorAndPowerProfile {
    ...
    $defaultProfile = New-WinMintInstallPlanSetupProfile -BuildConfig $defaultConfig
    ...
    $ultimateSetup = New-WinMintInstallPlanSetupProfile -BuildConfig $ultimateConfig
...
function Initialize-WinMintSmokeBuildManifest {
...
function Assert-ManifestPayloadsAreDeduplicated {
    ...
    try {
        ...
        Initialize-WinMintSmokeBuildManifest -Config $config
...
function Assert-TweakAuditArtifactsAreWritten {
    ...
    try {
        ...
        $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
        if (-not $manifest.audit -or @($manifest.audit.records).Count -lt 1) {
            Add-SmokeFailure 'Expected manifest audit.records to embed build delta records.'
        }
        ...
        if (@($manifest.audit.records).Count -eq 0) {
            Add-SmokeFailure 'Expected embedded audit records to contain at least one backend change record.'
        }
...
function Assert-CachedDownloadResolver {
    ...
    try {
        ...
        $missing = Get-WinMintCachedDownloadFile -DownloadDir $tempRoot -Patterns @('WinMint-NoSuchPayload-*.zip')
        if ($null -ne $missing) {
...
function Assert-OfflinePayloadCacheStatus {
    ...
    try {
        ...
        Remove-Item -LiteralPath (Join-Path $downloads 'Microsoft.DesktopAppInstaller_8wekyb3d8bbwe.msixbundle') -Force
        $incomplete = Get-WinMintOfflinePayloadCacheStatus -Architecture 'arm64' -DownloadDir $downloads -FontDir $fonts
        if ($incomplete.Complete -or @($incomplete.Missing) -notcontains 'winget') {
            Add-SmokeFailure 'Expected offline payload cache status to report missing winget.'
        }
    }
    finally {
        if (Test-Path -LiteralPath $tempRoot) {
...
function Assert-HeadlessCliContracts {
    ...
    try {
        ...
        if (-not [bool]$template.posture.setup.dmaInterop) {
            Add-SmokeFailure 'Expected generated profile templates to enable DMA interoperability by default.'
        }
        if ([string]$template.privacy.locationServices -ne 'enabled') {
    ...
    try {
        $null = Invoke-WinMintBuildCommand -ProfilePath 'config\build-profiles\surface-laptop-7-microsoft-oobe.json' -ComputerName 'Other'
    ...
    $dmaSetupProfile = New-WinMintInstallPlanSetupProfile -BuildConfig $dmaConfig
    ...
    Initialize-WinMintSmokeBuildManifest -Config $dmaConfig
    ...
    $dmaLocationSetupProfile = New-WinMintInstallPlanSetupProfile -BuildConfig $dmaLocationConfig
    ...
    $groupAgentProfile = New-WinMintInstallPlanAgentProfile -BuildConfig $groupConfig
    ...
    if ($groupConfig.RegistryTweaks -contains 'gamebar-policy') {
        Add-SmokeFailure 'Expected -KeepGaming to suppress gamebar-policy while keeping Xbox apps.'
    }
...
function Assert-UiBridgeBuildProfileContract {
    ...
    $null = New-Item -ItemType Directory -Path $tempRoot -Force
    try {
        $settingsPath = Join-Path $tempRoot 'wizard-settings.json'
        ...
        $badSettingsPath = Join-Path $tempRoot 'bad-wizard-settings.json'
        [ordered]@{
            Profile = 'WinMint'
            Architecture = 'amd64'
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                Add-SmokeFailure 'Expected UI bridge to reject wizard settings missing required keep-flag fields.'
            }
        }
        catch {
