...
Invoke-CliProfileCase -Name 'minimal-no-dma-no-location' -Arguments @('-Dma', 'Off', '-Location', 'Off') -Assert {
    param($Profile, $Config, $AgentProfile)
    [void]$AgentProfile
    if ([bool]$Profile.posture.setup.dmaInterop -or [bool]$Config.DmaInterop.Enabled) { Add-CliMatrixFailure 'minimal-no-dma-no-location should disable DMA interop.' }
    ...
    if ([string]$Profile.privacy.locationServices -eq 'enabled' -or [bool]$Config.Privacy.Location) { Add-CliMatrixFailure 'minimal-no-dma-no-location should disable location services.' }
