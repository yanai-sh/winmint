...
Assert-BootstrapText -Pattern "\[ValidateSet\('Gui','Headless','LegacyUi'\)\]" -Description 'explicit launcher mode set'
...
Assert-BootstrapText -Pattern '\[switch\]\$LegacyUi' -Description 'legacy UI launcher switch'
Assert-BootstrapText -Pattern 'WinMint-GUI\.ps1' -Description 'default GUI entry point'
Assert-BootstrapText -Pattern 'WinMint-LegacyUI\.ps1' -Description 'legacy UI entry point'
