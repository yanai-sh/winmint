...
/// <summary>S5 Host Apply seam: Apply workdir evidence assert (no Hyper-V, no install). Excluded from just check.</summary>
public class ApplyEvidenceAssertTests
{
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_passes_with_driver_inventory_and_digests()
    {
        ...
        try
        {
            ...
            string acceptancePath = Path.Combine(work, "apply-acceptance.json");
            Assert.True(File.Exists(acceptancePath), "expected apply-acceptance.json");
            string json = File.ReadAllText(acceptancePath);
            Assert.Contains("winmint.apply.acceptance/v1", json, StringComparison.Ordinal);
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_driver_digest_missing()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
        {
            ...
            Assert.False(File.Exists(Path.Combine(work, "apply-acceptance.json")));
        }
        finally
        {
            TryDelete(work);
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_firmware_would_be_injected()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_outputIso_digest_stale()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_RequireLane_mismatches()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_Release_packageStrict_missing()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_Release_packageStrict_false()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_fails_when_fu_digest_missing_on_ExpectFuPosture()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
    ...
    [Fact]
    [Trait("Category", "S5")]
    public void Assert_apply_evidence_passes_release_fu_posture_when_payload_complete()
    {
        string repo = TestRepo.Root;
        string work = CopyFixtureToTemp(repo);
        try
        {
            ...
            int exit = RunAssert(
                ...
                requireLane: "Release");
            Assert.True(exit == 0, $"exit={exit}\nstdout={stdout}\nstderr={stderr}");
            string acceptance = File.ReadAllText(Path.Combine(work, "apply-acceptance.json"));
    ...
    {
        string fixture = Path.Combine(repo, "tests", "fixtures", "apply-evidence");
    ...
    {
        string script = Path.Combine(repo, "tools", "apply", "Assert-ApplyEvidence.ps1");
        ...
        Assert.True(p.WaitForExit(60_000), "Assert-ApplyEvidence.ps1 timed out");
        return p.ExitCode;
    }
