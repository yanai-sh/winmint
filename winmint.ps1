...
function Find-WinMintWizardHost {
    param([string]$Root)

    $archFolder = if ($env:PROCESSOR_ARCHITEW6432) { $env:PROCESSOR_ARCHITEW6432 } else { $env:PROCESSOR_ARCHITECTURE }
    $archFolder = if ($archFolder -eq 'ARM64') { 'arm64' } else { 'x64' }
    $candidate = Join-Path $Root "assets\runtime\setup\setup-shell\bin\$archFolder\WinMintSetupShell.exe"
    if (-not (Test-Path -LiteralPath $candidate -PathType Leaf)) {
        throw "WinMintSetupShell.exe was not found under '$Root'. This release is missing the WebView2 wizard host."
    }

    return $candidate
}

function Find-WinMintWizardLauncherScript {
    param([string]$Root)
    ...
    throw "WinMint-GUI.ps1 was not found under '$Root'. This release is missing the packaged wizard launcher."
}
...
function New-WinMintWizardHostArguments {
    param([Parameter(Mandatory)][string]$Root)

    $shellRoot = Join-Path $Root 'assets\runtime\setup\setup-shell'
    return @(
        '--wizard',
...
function Test-WinMintInstalledVersion {
    ...
    if ([string]::IsNullOrWhiteSpace($guiScript) -or -not (Test-Path -LiteralPath $guiScript -PathType Leaf)) {
        Write-WinMintBootstrapLog "Existing version marker points to a missing wizard launcher; reinstalling." 'WARN'
        return $false
    }
...
try {
    ...
    if ($useInstalledVersion) {
        ...
        $guiScript = Find-WinMintWizardLauncherScript -Root $versionRoot
        if ($useDurableReleaseCache) {
    ...
    if ($NoLaunch) {
        ...
        if ($launchMode -eq 'Gui') {
            $wizardHost = Find-WinMintWizardHost -Root $versionRoot
            $wizardArguments = New-WinMintWizardHostArguments -Root $versionRoot
            $proc = Start-Process -FilePath $wizardHost -ArgumentList $wizardArguments -Wait -PassThru
            $scriptExitCode = $proc.ExitCode
            if ($scriptExitCode -ne 0) {
                Set-WinMintBootstrapOperation `
                    -Operation "Waiting for packaged wizard host '$wizardHost'." `
                ...
                throw "Packaged wizard host exited with code $scriptExitCode."
            }
        }
        else {
