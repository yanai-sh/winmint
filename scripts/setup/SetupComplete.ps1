...
# Maintain.ps1 = optional post-update AppX/policy drift control. Boot scheduled-task registration is
# off by default; set $true to register 'WinWSSlim-Maintain' at end of this script (see block below).
$RegisterWinWSMaintainScheduledTask = $false
...
function New-ScWingetInstallArgs {
    ...
    if ($wingetArchitecture) {
        $args += @('--architecture', $wingetArchitecture)
    }
    return $args
}
...
$scripts = @(
    ...
    }
    {
        try {
            ...
            if ($bitLockerVolume -and $bitLockerVolume.ProtectionStatus -eq 'On') {
                Write-ScLog 'Leaving active BitLocker protection enabled; WinWS only prevents automatic device encryption.'
            }
        }
        catch { }
    }
    ...
    {
        if (-not $RegisterWinWSMaintainScheduledTask) {
            return
        }
        $maintScript = 'C:\Windows\Setup\Scripts\Maintain.ps1'
        if (Test-Path -LiteralPath $maintScript) {
            $pwsh = "$env:ProgramFiles\PowerShell\7\pwsh.exe"
            $exe  = if (Test-Path $pwsh) { $pwsh } else { 'powershell.exe' }
            $action   = New-ScheduledTaskAction -Execute $exe -Argument "-NonInteractive -WindowStyle Hidden -File `"$maintScript`""
            $trigger  = New-ScheduledTaskTrigger -AtStartup
            $settings = New-ScheduledTaskSettingsSet -ExecutionTimeLimit (New-TimeSpan -Minutes 5) -StartWhenAvailable
            Register-ScheduledTask -TaskName 'WinWSSlim-Maintain' -Action $action -Trigger $trigger `
                -RunLevel Highest -User 'SYSTEM' -Settings $settings -Force -ErrorAction SilentlyContinue | Out-Null
        }
    }
)
