...
function Read-WinWSFirstLogonSetupProfile {
    ...
    try {
        ...
        Write-WinWSFirstLogonError "Setup profile read failed: $_"
    }
    return $null
}

function Get-WinWSFirstLogonNestedProfileValue {
    param(
        [object]$Profile,
        [string]$Section,
        [string]$Nested,
...
function Resolve-WinWSPowerShellHost {
...
function Resolve-WinWSFirstLogonAgentMode {
    param(
        [Parameter(Mandatory)][string]$RequestedMode,
        [Parameter(Mandatory)][string]$AgentUIPath
    )

    $envMode = [string]$env:WINWS_FIRSTLOGON_MODE
...
function Invoke-WinWSFirstLogonReg {
    param(
    ...
    if ($LASTEXITCODE -ne 0 -and -not $AllowFailure) {
        Write-WinWSFirstLogonError "reg.exe $($Arguments -join ' ') exited $LASTEXITCODE`n$($out | Out-String)"
    }
}

function Set-WinWSFirstLogonDesktopDefaults {
    $wallpaperPath = 'C:\Windows\Web\Wallpaper\WinWS\WinWS-Bloom-OLED.png'
    $desktopKey = 'HKCU\Control Panel\Desktop'
    $personalizeKey = 'HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize'
    $dwmKey = 'HKCU\Software\Microsoft\Windows\DWM'

    Invoke-WinWSFirstLogonReg -Arguments @('add', $personalizeKey, '/v', 'AppsUseLightTheme', '/t', 'REG_DWORD', '/d', '0', '/f') -AllowFailure
    Invoke-WinWSFirstLogonReg -Arguments @('add', $personalizeKey, '/v', 'SystemUsesLightTheme', '/t', 'REG_DWORD', '/d', '0', '/f') -AllowFailure
    Invoke-WinWSFirstLogonReg -Arguments @('add', $dwmKey, '/v', 'ColorPrevalence', '/t', 'REG_DWORD', '/d', '0', '/f') -AllowFailure

    if (Test-Path -LiteralPath $wallpaperPath) {
        Invoke-WinWSFirstLogonReg -Arguments @('add', $desktopKey, '/v', 'Wallpaper', '/t', 'REG_SZ', '/d', $wallpaperPath, '/f') -AllowFailure
        Invoke-WinWSFirstLogonReg -Arguments @('add', $desktopKey, '/v', 'WallpaperStyle', '/t', 'REG_SZ', '/d', '10', '/f') -AllowFailure
        Invoke-WinWSFirstLogonReg -Arguments @('add', $desktopKey, '/v', 'TileWallpaper', '/t', 'REG_SZ', '/d', '0', '/f') -AllowFailure
        Add-Type -Namespace WinWS.Native -Name User32 -MemberDefinition @'
...
'@ -ErrorAction SilentlyContinue
        [WinWS.Native.User32]::SystemParametersInfo(20, 0, $wallpaperPath, 3) | Out-Null
    }
}

function Set-WinWSFirstLogonRetry {
    $runOnce = 'HKCU\Software\Microsoft\Windows\CurrentVersion\RunOnce'
    $exe = Resolve-WinWSPowerShellHost
    $command = "`"$exe`" -NoLogo -NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`""
    Invoke-WinWSFirstLogonReg -Arguments @('add', $runOnce, '/v', 'WinWSFirstLogonRetry', '/t', 'REG_SZ', '/d', $command, '/f')
}

function Clear-WinWSFirstLogonRetry {
    $runOnce = 'HKCU\Software\Microsoft\Windows\CurrentVersion\RunOnce'
    Invoke-WinWSFirstLogonReg -Arguments @('delete', $runOnce, '/v', 'WinWSFirstLogonRetry', '/f') -AllowFailure
}

function Clear-WinWSAutoLogonPassword {
    ...
    foreach ($name in @('DefaultPassword', 'AutoLogonCount')) {
        Invoke-WinWSFirstLogonReg -Arguments @('delete', $winlogon, '/v', $name, '/f') -AllowFailure
    }
}

function Restore-WinWSFirstLogonRegionalDefaults {
    $setupProfile = Read-WinWSFirstLogonSetupProfile
    $dmaInterop = [bool](Get-WinWSFirstLogonNestedProfileValue -Profile $setupProfile -Section 'regional' -Nested 'dmaInterop' -Name 'enabled' -Default $true)
    if (-not $dmaInterop) { return }

    $restoreGeoId = [int](Get-WinWSFirstLogonNestedProfileValue -Profile $setupProfile -Section 'regional' -Nested 'dmaInterop' -Name 'restoreHomeLocationGeoId' -Default 244)
    $restoreUserLocale = [string](Get-WinWSFirstLogonNestedProfileValue -Profile $setupProfile -Section 'regional' -Nested 'dmaInterop' -Name 'restoreUserLocale' -Default '')
    try {
        ...
        Write-WinWSFirstLogonError "Home location restore failed for GeoID ${restoreGeoId}: $_"
    }
    if (-not [string]::IsNullOrWhiteSpace($restoreUserLocale)) {
        try {
            ...
            Write-WinWSFirstLogonError "User culture restore failed for ${restoreUserLocale}: $_"
        }
    }
}

function Invoke-WinWSFirstLogonOneDriveRemoval {
    ...
    foreach ($setup in @(
        ...
        if (Test-Path -LiteralPath $setup) {
            try {
                ...
                Write-WinWSFirstLogonError "OneDrive uninstall failed for ${setup}: $_"
            }
        }
    }
    ...
    foreach ($setupFile in $setupFiles) {
        ...
        try {
            ...
            Write-WinWSFirstLogonError "OneDrive installer file removal failed for ${setupFile}: $_"
        }
    }
    ...
    foreach ($known in $knownFolders) {
        ...
        Invoke-WinWSFirstLogonReg -Arguments @('add', $userShellFolders, '/v', $known.Name, '/t', 'REG_EXPAND_SZ', '/d', $expandValue, '/f') -AllowFailure
        Invoke-WinWSFirstLogonReg -Arguments @('add', $shellFolders, '/v', $known.Name, '/t', 'REG_SZ', '/d', $absoluteValue, '/f') -AllowFailure
    }
    ...
    foreach ($args in @(
        ...
        Invoke-WinWSFirstLogonReg -Arguments $args -AllowFailure
    }
    ...
    foreach ($runKey in @(
        ...
        foreach ($value in @('OneDrive', 'OneDriveSetup')) {
            Invoke-WinWSFirstLogonReg -Arguments @('delete', $runKey, '/v', $value, '/f') -AllowFailure
        }
    }
        ...
        foreach ($key in @(Get-ChildItem -LiteralPath $root -ErrorAction SilentlyContinue)) {
            ...
            if ($text -match '(?i)OneDrive|OneDriveSetup\.exe') {
                try {
                    ...
                    Write-WinWSFirstLogonError "OneDrive registry residue removal failed for $($key.Name): $_"
                }
            }
        }
    }
    ...
    foreach ($root in @($oneDriveRoots | Where-Object { $_ -and (Test-Path -LiteralPath $_) })) {
        ...
        if ($children.Count -eq 0) {
            ...
            Write-WinWSFirstLogonError "OneDrive folder not removed because it contains files: $root"
        }
    }
    ...
    $oneDriveAudit = [ordered]@{
        ...
        namespacePinned = [ordered]@{
            clsid64 = (Get-ItemProperty -LiteralPath 'Registry::HKEY_CLASSES_ROOT\CLSID\{018D5C66-4533-4307-9B53-224DE2ED1FE6}' -Name 'System.IsPinnedToNameSpaceTree' -ErrorAction SilentlyContinue).'System.IsPinnedToNameSpaceTree'
            clsid32 = (Get-ItemProperty -LiteralPath 'Registry::HKEY_CLASSES_ROOT\WOW6432Node\CLSID\{018D5C66-4533-4307-9B53-224DE2ED1FE6}' -Name 'System.IsPinnedToNameSpaceTree' -ErrorAction SilentlyContinue).'System.IsPinnedToNameSpaceTree'
        }
...
function Disable-WinWSAutoAdminLogon {
    $winlogon = 'HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon'
    Invoke-WinWSFirstLogonReg -Arguments @('add', $winlogon, '/v', 'AutoAdminLogon', '/t', 'REG_SZ', '/d', '0', '/f')
}

function Remove-WinWSResidualPayload {
    # Pristine cleanup after a successful agent run. Keep Maintain.ps1 and WinWSSetupProfile.json
    # (Maintain can be run manually or re-enabled via SetupComplete.ps1). Everything else in
    # C:\Windows\Setup\Scripts is one-shot setup payload.
    $keep = @('Maintain.ps1', 'WinWSSetupProfile.json')
    ...
    foreach ($entry in @(Get-ChildItem -LiteralPath $payloadDir -Force -ErrorAction SilentlyContinue)) {
        ...
        catch { Write-WinWSFirstLogonError "Cleanup failed for $($entry.FullName): $_" }
    }
    ...
    } catch { Write-WinWSFirstLogonError "Self-delete schedule failed: $_" }
}
...
try { Save-WinWSFirstLogonState -State $state } catch { Write-WinWSFirstLogonError "FirstLogon state write failed: $_" }
try { Set-WinWSFirstLogonRetry } catch { Write-WinWSFirstLogonError "FirstLogon retry registration failed: $_" }
...
try { Clear-WinWSAutoLogonPassword } catch { Write-WinWSFirstLogonError "AutoLogon password clear failed: $_" }
try { Set-WinWSFirstLogonDesktopDefaults } catch { Write-WinWSFirstLogonError "Desktop defaults failed: $_" }
try { Invoke-WinWSFirstLogonOneDriveRemoval } catch { Write-WinWSFirstLogonError "OneDrive user cleanup failed: $_" }

$agentExitCode = 0
$agentRoot = Join-Path $payloadDir 'WinWSAgent'
$agent     = Join-Path $agentRoot 'Start-WinWSAgent.ps1'
$agentUI   = Join-Path $agentRoot 'Start-WinWSFirstLogonUI.ps1'
if (Test-Path -LiteralPath $agent) {
    try {
        $exe = Resolve-WinWSPowerShellHost
        # The agent is the source of truth and can run without any GUI. UI and
        # console modes are observers over state.json and WinWSAgent-events.jsonl.
        $mode = Resolve-WinWSFirstLogonAgentMode -RequestedMode $AgentMode -AgentUIPath $agentUI
        "$(Get-Date -Format 'o') Launching WinWSAgent in $mode mode" | Out-File (Join-Path $logDir 'FirstLogon.log') -Append
        if ($mode -eq 'UI') {
        ...
        if ($agentExitCode -ne 0) { Write-WinWSFirstLogonError "WinWSAgent exited with code $agentExitCode" }
    }
    catch {
        $agentExitCode = 1
        Write-WinWSFirstLogonError "WinWSAgent launch failed: $_"
    }
}
else {
    $agentExitCode = 1
    Write-WinWSFirstLogonError "WinWSAgent script was not found: $agent"
}
...
if ($agentExitCode -eq 0) {
    $state['status'] = 'ok'
    try {
        Restore-WinWSFirstLogonRegionalDefaults
        Clear-WinWSFirstLogonRetry
        Disable-WinWSAutoAdminLogon
    }
    catch {
        Write-WinWSFirstLogonError "AutoLogon cleanup failed: $_"
    }
    try { Remove-WinWSResidualPayload }
    catch { Write-WinWSFirstLogonError "Residual cleanup failed: $_" }
}
else {
    $state['status'] = 'failed'
    Write-WinWSFirstLogonError 'AutoAdminLogon left enabled so the next sign-in can retry; the password value has already been cleared.'
}
try { Save-WinWSFirstLogonState -State $state } catch { Write-WinWSFirstLogonError "FirstLogon state write failed: $_" }
