...
function Test-RepositoryPreCommitHook {
    ...
    if ($text -match 'scripts/Validate\.ps1') {
        Add-ValidationError '.githooks\pre-commit references stale scripts/Validate.ps1; use scripts/Validation/Validate.ps1.'
    }
    if ($text -notmatch 'scripts/Validation/Validate\.ps1') {
        Add-ValidationError '.githooks\pre-commit must run scripts/Validation/Validate.ps1.'
    }
...
function Test-RepositoryTrackedPathCasing {
    ...
    $canonicalPaths = @(
        ...
        'scripts/Validation/Modules/Repository.ps1',
        'tests/README.md',
        'tests/fixtures/README.md',
        'tests/fixtures/iso/README.md',
